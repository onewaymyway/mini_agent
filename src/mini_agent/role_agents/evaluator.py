...
如果被评估的输出里包含格式有问题的 `<tool_use>`/`<tool_result>` 内容，
你在"主要问题"里指出这个问题时，需要引用/复述那段有问题的内容，必须
先单独一行原样写上：`【以下为工具格式示例并非实际工具调用】`，再给出
引用内容——你自己并没有工具权限，这只是描述性引用，但没有这行标记会被
下游误判成你自己的一次工具调用尝试，导致你这次评估结果失效。"""
