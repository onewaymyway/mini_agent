...
*2026-08 真正接入 explore_runner（text-core 真实实现）*：新增 `skills/generative_capability/real_tools.py`：`text_transform_apply` 真实实现纯字符串变换（upper/lower/reverse/strip/title/capitalize/swapcase/append/prepend/replace，可组合调用），`build_default_tool_executor()` 按工具名分发，未接入的工具（`browser-core`/`doc-core`）如实报错不伪造成功。`tools/capability_call.py` 默认注入该 executor 及基于它构造的 `explore_runner`（此前"故意不注入"），`text-transform-capability` 由此成为第一个能在真实对话里跑通完整 explore->distill->落盘复用 全链路的 generative-capability skill；`browser-site-scraper`/`doc-template-generation` 因原语仍是占位，行为不变（诚实 `not_implemented`）。新增 `tests/test_generative_capability_real_tools.py` + `TestRealTextCoreExploreEndToEnd`（脚本化假 LLMHelper 驱动真实决策循环，不发真实网络请求）。详见 [方案文档阶段十二](next_doc/generative-capability-skill-plan.md)。

*2026-08 generative-capability `request_formats`：请求形状校验*：`capability.yaml` 新增可选字段 `request_formats`（每种形状给 `required_fields` + 可照抄的 `example`），`CapabilityEngine.call()` 在 `resolve()` 之前先做一次零成本形状检查，request 不满足任何声明格式时直接返回新状态 `invalid_request`（`data.expected_formats` 带回全部候选格式），不再误落入 `no_match -> explore -> not_implemented` 让调用方误以为"能力未实现"；未声明该字段的 skill 行为不变。`tools/capability_call.py` 同步透传新状态。已给 `browser-site-scraper`/`doc-template-generation`/`text-transform-capability` 三个 skill 补上声明；`tests/test_generative_capability_engine.py` 新增 `TestRequestFormatValidation` / `TestRequestFormatBackwardCompat`。详见 [方案文档阶段十一](next_doc/generative-capability-skill-plan.md)。

*2026-08 修复 `view_raw_result` 结果被二次截断*：`tool_executor.py::_trim_result` 对 `tool_name == "view_raw_result"` 直接原样返回（不再进入规则截断/LLM 摘要/RawResultStore 流程），命令行渲染侧 `execute_all()` 调用 `print_tool_result` 时对该工具传 `truncate=None`（不再套用默认 2000 字符预览截断）。修复前该工具的返回结果本身超过截断阈值时会被再截一次，导致"回看完整原文"实际上还是残缺的；现在无论 agent 拿到的结果还是命令行打印出来的结果都是完整原文。详见 [工具结果原始留存与智能摘要指南](docs/tool-result-raw-store-and-smart-summary-guide.md)。
