...
2026-08-27 追加：`tools/fetch_iwencai_cookie.py` 判断"用户是否已完成
登录/验证"的逻辑分交互式/非交互式两条路径（详见该文件模块 docstring
"2026-08-27 再次追加"一节）。经这层包装、被看板「▶️ 手动触发」调用时，
子进程的 stdin 通常不是真正的终端（`sys.stdin.isatty()` 为
`False`），会自动落到**非交互式**那条路径——轮询等待令牌值相对"页面刚
打开时的基线值"发生变化，而不是等回车确认。这仍然比"cookie 一出现就
当作成功"可靠，但不如人在终端里直接跑 `tools/fetch_iwencai_cookie.py`
按回车确认来得确定；如果通过看板触发总是在没真正登录的情况下就超时/
拿到旧值，建议改成在有桌面环境的终端里直接跑 `tools/` 下的脚本。
