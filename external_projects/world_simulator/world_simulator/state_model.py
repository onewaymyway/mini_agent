...
class SimState:
    ...
    **刻意不做的部分**（详见方案 2.3 节"范围克制"）：不做"能力→
    行动空间自动扩展"的关联计算；不做任何跨步骤的能力判重/合并
    （同 `problems.id` 的既有取舍，"这是不是同一个能力的延续"完全
    交给 LLM/用户判断——展示层按 `capability` 精确字符串匹配归并，
    见下方 `maturity_stage`）。

    默认空列表：skill 没给出、旧数据、多数平淡的推进步骤都可以为空，
    不影响任何已有行为，向后兼容。

    每一项再额外支持一个可选字段 `maturity_stage`（第十一轮 2.3
    节，`next_doc/world_simulator_eleventh_round_remaining_gaps_
    plan.md`，对照参考文档第十二、十三、十六节"技术/能力应该有
    阶段性生命周期"）——参考文档原文是不可行→实验室可行→专家可用→
    开发者可用→普通用户可用→成本足够低→大规模应用→基础设施化→
    社会常态化九段，这里**精简成六段枚举**（同项目一贯"精简子集，
    不照搬全部术语"的取舍，类比 `problems.status` 只取四段）：

    - `"lab"`：实验室可行
    - `"expert"`：专家可用
    - `"developer"`：开发者可用
    - `"consumer"`：普通用户可用
    - `"cheap_at_scale"`：成本足够低、可大规模应用
    - `"infrastructure"`：基础设施化，含参考文档"社会常态化"最后
      一段——两段界限在叙事层面很难清晰区分，合并成一段更符合
      "不认识的取值原样保留、不强行精确分类"的既有风格

    不认识的取值原样保留、不做校验（同 `problems.status`/
    `relationship.py::reversibility` 的既有取舍一致）；不填表示
    "这一步没有明确判断出所处阶段"。

    **不做**自动阶段推进算法——阶段变化仍然完全由 LLM 在叙事里体现、
    由 skill 声明，`engine.py` 只做存储透传，不做"满足什么条件就
    自动升级到下一阶段"的判断（同 `problems.status` 的既有取舍一致，
    不引入这个项目一直刻意回避的"规则引擎"）。

    同一项能力（按 `capability` 字段精确字符串匹配，不做模糊匹配/
    语义归并）可能在不同步骤被反复提及、给出不同的 `maturity_
