...
- 2026-09-24（第二十轮，用户反馈直接改进，未单独立项 next_doc）：
  **"模拟了几十步，🕸️ 问题关系图还是空的"——把"问题自动扫描"从默认
  关闭改成默认开启**。
  排查结论（和 field_ledger 那次不同，这次不是"该做但漏做"，而是
  "兜底通道本身默认没开"）：`problems` 字段完全靠模型在 `advance_
  step` 里自愿声明，prompt 里这类"稀疏声明"可选字段有五六个之多，
  模型大概率会系统性跳过；项目里专门做了 Problem Discovery Engine
  的自动周期扫描（`_safe_auto_scan_problems()`，第十轮批次一）作为
  不依赖模型自觉的兜底通道，但 `manifest.settings["problem_
  discovery_auto_scan_interval"]` 默认是 `0`（关闭），新建实例不会
  自动打开，用户也不一定知道要去设置里手动开——几十步一直空着，是
  这个兜底通道从头到尾没被触发过。
  1. **`world_simulator/problem_discovery.py`**：新增
     `DEFAULT_AUTO_SCAN_INTERVAL = 5` 和 `get_effective_auto_scan_
     interval(settings)`——`settings` 里完全没有这个 key（新建实例、
     或从未在设置面板里保存过的旧实例）时返回默认值 `5`；显式存了
     这个 key（不管是 `0` 还是其它正整数）时尊重这个显式值，解析
     失败（脏数据）才退回默认值。这样区分"从未配置过"和"用户主动
     设成 0 关闭"，不会覆盖显式选择，同时新建实例、以及大量从来没
     打开过"模拟设置"面板的旧实例，都能不做任何操作就自动获得这个
     默认行为，不需要用户自己发现并手动打开开关。`_safe_auto_scan_
     problems()` 的间隔解析改用这个共享函数（原来是 `int(settings.
     get(...) or 0)`，把"未配置"和"显式 0"都当 0 处理，没法区分）。
     顺手改掉了模块顶部一条过时的"范围克制"说明（写着"不做自动周期
     性触发"，但第十轮批次一早就加了自动周期触发，这条描述此前一直
     没更新）。
  2. **`app.py`**：设置面板"问题自动扫描间隔"的数字输入框默认显示值
     和标签文案都改用 `get_effective_auto_scan_interval()`，保证
     "UI 上看到的默认值"和"引擎实际使用的默认值"一致（之前是 UI 显示
     0、引擎现在实际按 5 生效这种错位，容易误导用户）。`_render_
     problem_graph_section()` 新增 `auto_scan_interval` 参数，空
     状态提示里追加一句：如果这个实例的自动扫描确实被显式关闭了
     （`<= 0`），额外提示"去模拟设置把间隔调成正整数"，把排查路径
     直接摆在用户面前而不是要求用户自己去找。
  3. **测试**：`tests/test_problem_discovery.py` 新增/改写共 6 个
     用例——`get_effective_auto_scan_interval()` 的四种情形（缺省/
     显式 0/显式正整数/脏数据兜底），以及把原来那条"未配置=不触发"
     的用例拆成两条更准确的（"显式 0 即使撞上默认间隔的整数倍也不
     触发"/"未配置且 `step` 是默认间隔 5 的整数倍时应该触发"），
     原用例的断言在新默认值下"碰巧还是通过"但名字和意图已经不对，
     没有直接留着。
  **验收**：`pytest tests/`（692 passed，含本轮新增/改写 6 个用例），
