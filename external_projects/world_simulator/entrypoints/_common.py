...
职责：
  1. 把项目根加入 `sys.path`，使得 `import world_simulator.xxx` 在直接
...
def tracked_run(entrypoint: str, *, trigger: str = "external_cron") -> Iterator[object]:
    ...
    try:
        ...
        logging.getLogger("world_simulator").info(
...
def append_backlog(summary: str, *, source: str = "outcome_review", evidence_ref: str = "") -> None:
    ...
    try:
        ...
        logging.getLogger("world_simulator").warning("写入改进积压账本失败（已忽略）: %s", exc)
