...
限速到不可用。所以 `validator.py` 里是"起本地 xray 进程 -> 真实发一次 HTTP
请求(默认打 `www.gstatic.com/generate_204`)-> 记录延迟",这样得到的可用性
判断才是准的。代价是验证一批节点比单纯 ping 慢,所以做了并发度限制
(`concurrency` 参数)避免同时拉起太多 xray 子进程。
