...
巩固循环 是 mini_agent 自我演化系统的**后台循环**：定期扫描已有数据，主动识别"可以改进的机会"。它由四个扫描任务组成，通过一个时间门控机制触发，**不需要常驻进程**。

```
触发方式（二选一）：
  A) 手动：/evolve consolidate
...
## 2. 手动触发：/evolve consolidate

```bash
/evolve consolidate            # 标准运行（受时间门控保护）
/evolve consolidate --force    # 忽略 24h 限制，强制运行
/evolve consolidate --dry-run  # 只展示报告，不写入节奏治理记录
```
...
每次 `trigger_session_end()` 时，`_maybe_run_consolidation()` 检查 `.agent/consolidation_rhythm.json` 里的 `_last_run_at`：

```
elapsed = now - last_run_at
if elapsed >= 24h:
    run_consolidation()  # 后台运行，有发现时打印单行摘要
    # "[consolidate] 发现 2 个剪枝候选，用 /evolve consolidate 查看详情。"
```
...
> **不自动删除**：巩固循环 只输出候选列表，不执行任何 skill 的停用操作。需要人工确认后通过 `/skill deactivate <name>` 下线。
...
**晋升不自动发生**：巩固循环 只输出候选列表，真正的 skill 提案需要手动运行 `/evolve review` 触发 evolution-agent 来完成。这遵循"高风险操作需人工确认"的一贯原则（T1 级别）。
...
巩固循环 使用 `.agent/consolidation_rhythm.json` 记录提案历史：
...
每次 巩固循环 运行后，对**已生成**的候选记录时间戳。下次运行时，如果某条候选的上次提案时间距今不足 7 天，则跳过（不再出现在报告里）。

### 5.2 观察期（T1 安全网）

设计文档第 18 节开放问题 1 的回答：**T1 级别的自动合并需要先观察 5 个 session**（`observation_window_sessions=5`，可配置）。这个参数当前记录在晋升候选的元数据里，evolution-agent 在决定是否自动提案时会读取，不在 巩固循环 本身做阻塞。
...
Stage 4 W2 (work_index / open_threads)
  └─ 提供 manifest.json 数据 ──→ 巩固循环 8.3 能力地图

Stage 5 W3 (cross_project_index)
  └─ 提供跨项目模式 ────────→ 巩固循环 8.4 晋升候选

Stage 6 (observability / traces.jsonl)
  └─ 提供 context_breakdown ──→ 巩固循环 8.2 token 成本估算（未来扩展点）
...
| 功能 | 位置 |
|------|------|
| 整体模块 | `src/mini_agent/evolution/consolidation.py` |
| `run_consolidation()` | 整体运行入口 |
| `should_run_consolidation()` | 时间门控检查（24h）|
...
### Q: `/evolve consolidate` 什么都没输出？

时间门控在起作用——24h 内已经运行过了。用 `--force` 强制运行，或等到下次 session 结束时自动触发。

### Q: 剪枝候选里的 skill，删除它安全吗？

巩固循环 只给**建议**，不执行操作。用 `/skill info <name>` 看详情，确认无误后用 `/skill deactivate <name>` 停用（不删除文件）。
...
### Q: cross_project_index 为空，巩固循环 没发现晋升候选？
...
- [命令与工具参考](commands-and-tools-reference.md) — `/evolve consolidate` 命令参数
- [四项优先改进指南](four-priority-improvements-guide.md) — Affordance 权重闭环校准（`calibrate()`，巩固循环 新增步骤）
