...
## 5.6 自主检索与素材沉淀改进（阶段一/阶段二，`next_doc/growth_advisor_autonomous_search_and_material_improvement_plan.md`）

主动检索能力（`report_active_search_enabled`/`cron_triggered_active_
search_enabled`）此前只做到"从无到有查一次"，检索质量和素材利用率都比较
初级。本轮针对"查得深不深、查到的东西用没用上"做了两处改进，**默认行为
完全不变**，只有显式调整相关配置项才会有差异：
...
- 检索触发条件仍然是"被动扫描命中数是否为 0"，不会因为外部世界发生了
  新变化而对已有素材的主题重新检索；
- 报告仍然是单一 ≤500 字的简报模板，"报告"（决策向）和"学习素材"
  （执行向、结构化路径）没有区分；
- 报告正文写完后不会回头核对是否真的引用了摘录内容，标注来源仍然只是
  在 prompt 里"要求"模型做，没有验证步骤。
