...
> 3. 当前消息以续接动词类短语开头（`"继续"`/`"接着"`/`"往下"`/`"go on"`/`"go ahead"`/
>    `"continue"`/`"keep going"`/`"proceed"`）——即使后面带了具体任务描述（如"继续想
>    办法解决 XXX 问题"，整句不在白名单里），前缀已足以表达"延续当前话题"，同样直接
>    判定非切换。
> 起因：`"关键词重合度 0% 低于阈值 15%"` 曾把"继续"这类续接指令误判为话题切换；
> 后续发现"继续 + 具体任务描述"这类长句因为不整句匹配白名单，仍会被误判，故补充第 3 条。

> **LLM 二次确认的上下文范围（2026-07 三次更新）**：`llm` 档最初只把"上一轮 vs 当前轮"
> 这两条用户消息喂给小模型判断，容易漏判"中途绕开处理了个子问题、又绕回最初任务"这类
> 场景（相邻两轮字面差异大，但结合更早的输入看其实一直是同一件事）。现在改为
> `TopicShiftTrigger._llm_confirm_with_context()`：以 active history 中最近一条压缩占位符
> （`compressed`/`compact_summary`/`session_resume`，本次会话若还没 compact 过则视为从头开始）
> 为边界，收集**自上次 compact 以来的全部用户输入**（按时间顺序，超过 `_MAX_CONTEXT_USER_TURNS`
