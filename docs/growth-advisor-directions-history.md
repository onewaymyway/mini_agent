# 成长顾问（Growth Advisor）演进日志

> 本文档是 [growth-advisor-guide.md](growth-advisor-guide.md) 的配套演进日志，
> 按批次（大致对应时间先后）记录各能力方向"何时因为什么方案落地、具体做了
> 什么"。主指南只保留稳定不变的核心机制、默认行为与配置项；某个方向的
> **产生动机、设计取舍、实现细节**都在这里。
>
> 拆分动机与规则见
...
> 本文档由主指南原 §2.5-2.24a、§5.5-§5.9 整体迁移而来，小节编号沿用原文档
> 编号（如 "2.9 节"），供其余文档里已有的编号引用继续有效；分组标题按
> 对应的 `next_doc/*_plan.md` 方案文件归批，是各批次落地的大致先后顺序，
> 不是精确日期。

## 批次一：P4-P6 基线完善（关键词/存储卫生/去重/反馈粒度/LLM 可观测性）
...
## 批次二：Goal/Cron 打通（`growth_advisor_goal_cron_integration_plan.md`）
...
## 批次三：调研信息获取与整理（`growth_advisor_research_quality_plan.md`）
...
## 批次四：cron 主动检索预算 / 检索质量反馈 / Goal 状态历史
...
## 批次六：自主性深化第一轮（`growth_advisor_autonomy_deepening_plan.md`：A1/A2/B1/B2/D1/D2/C1/C2/A3）
...
## 批次七：自主性深化第二轮（`growth_advisor_autonomy_deepening_plan_v2.md`：方向 1-5）
...
## 批次八：理想顾问差距与路线图（`growth_advisor_ideal_advisor_gap_and_roadmap_plan.md`：方向 4/5/6/7）
...
## 批次九：v4 新增能力（`growth_advisor_improvement_plan_v4.md`：N1-N4）

## 5.5 v4 新增能力（N1~N4，`next_doc/growth_advisor_improvement_plan_v4.md`）

在 P1~P6（[growth-advisor-guide.md](growth-advisor-guide.md) §3-5 描述的基线）之上，v4 计划的四个方向已全部落地
...
## 批次十：诊断快照性能优化（`session_backfill_index_incremental_plan.md`）
