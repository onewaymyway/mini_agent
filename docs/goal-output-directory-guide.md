...
### 11.5 已存在的旧 Goal 怎么办（自动补齐检测）

`user_output_dir`/`user_output_dir_suggested` 是后加的两个字段，这个功能
上线之前创建的 Goal（或者是从旧版本 `goals.json` 加载进来的）不会有这两
个字段，也就没机会在创建时跑过 §11.2 的检测。这部分**不需要手动迁移**：
每次加载 `goals.json`（`GoalBacklog.load()`，包括看板每次刷新背后的
`GET /v1/goals` 请求）都会自动检查每个 Goal——只要 `user_output_dir_
suggested` 还是"从未检测过"的状态（内部用 `None` 表示，区别于"检测过但
没找到"的空字符串），就会对它的 `description` 补跑一次同样的检测，结果
直接体现在这次返回给看板的数据里。

也就是说：**打开一次看板，旧 Goal 就会自动带上建议值（如果描述里确实有
路径提示的话）**，跟新建 Goal 的体验完全一致，不需要跑任何迁移命令。
补齐的结果只在内存里生效，不会单独触发一次磁盘写入（避免绕开正常的
加锁写入保护、和其他并发写入产生冲突）——真正落盘会随着这个 Goal 之后
第一次正常的写操作（比如你在看板上确认了建议值、改了优先级、或者这个
Goal 下一轮触发）自然一起完成，不需要用户关心这个细节。

同样地，这个自动补齐**只影响建议字段 `user_output_dir_suggested`**，不
会替你自动设置 `user_output_dir` 本身——真正生效仍然需要你在看板上确认
一次，跟 §11.2 的流程完全一样，只是不用重新手打一遍路径。
