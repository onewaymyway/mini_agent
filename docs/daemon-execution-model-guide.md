...
> **任务卡死时的恢复**：某个 step 执行期间遇到问题，表现分两种，daemon
> 都能正常继续跑，不会因此挂起或影响其它 Objective：
> - **抛异常**（工具调用报错、`run_turn()` 内部异常等）：会被立即捕获，
>   走 `on_failed` 回调，进入既有的重试/重新分解逻辑，对应线程立刻可以
>   被复用，不需要等待任何超时。
> - **真正卡死**（网络请求挂起、模型侧死循环等，`run_turn()` 既不返回
>   也不抛异常）：由 `reap_stale_steps()` 的存活性回收机制兜底——超过
>   `DEFAULT_STEP_TIMEOUT_SECONDS`（默认 600 秒）仍未收到执行结果的
>   step 会被判定为"已卡死"，走重试/终止流程。持久 Worker 模式下，
>   判定卡死后会立即释放该 execution 当前的专属线程池（Python 无法强制
>   杀死卡死的线程，但会不再引用它，改用一个全新的线程池重试），确保
>   重试不会被排到那条卡死线程背后永远排不上（见方案文档 §7.5）。
