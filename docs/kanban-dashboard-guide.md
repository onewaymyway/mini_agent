...
| 📥 主动建议 | 成长顾问 / 能力学习 / soft_goal_deriver 三条主动性管线候选的统一只读收件箱，按分类筛选，具体处理仍跳转到对应原生 tab（见下方专节） |
...
- **✏️ 编辑器视图**（`next_doc/workflow_visual_editor_plan.md`）：顶部「▶️ 运行与记录 / ✏️ 编辑器」
  单选框切换。直接编辑工作流 DAG，保存时保留原 YAML 的注释 / include 引用 / 相对路径：
  - **画布 + 属性面板**：已安装可选依赖 `streamlit-flow-component` 时用自动分层画布，点击节点选中、
    拖线加依赖、点边删依赖（merge_sources 虚线边只读）；未安装时降级为 `st.graphviz_chart` 只读图 +
    节点/边下拉选择器，功能完整只是没有拖拽。任何编辑都可以只靠属性面板完成，画布只是选择/连线的
    快捷方式。
  - **节点操作**：新增（按选中节点后插入）、复制、删除（删除前展示会同步移除的依赖 / merge 引用 /
    需要手动处理的 condition 引用）。
  - **🔗 依赖边操作**：下拉选一条依赖边，删除，或在这条边中间插入一个新节点（自动接管原有依赖关系）。
  - **并行批次高亮预览**：下拉选一个批次，画布把该批次的节点加粗高亮、其余淡化，并提示哪些节点因
    `allow_parallel=false` 会被单独串行。
  - **🧪 单步试运行**：选中节点后展开，用手工填写的 mock 数据（面板会按 step 里引用的占位符自动生成
    骨架）代替真实上游依赖，真实调用 LLM / 工具跑这一步——用的是**已保存**的定义，有未保存修改时禁用
    并提示先保存。依赖真实落盘文件的引用（如 `{id.output_file}`）无法伪造，会提示改用「续跑」验证。
  - **✅ 校验 / 💾 保存**：保存前校验（错误标红到具体节点）、`base_hash` 乐观锁冲突提示（重新加载 /
    强制覆盖）、注释丢失确认（未装 `ruamel.yaml` 时）；「🔍 查看变更」按文件展示 unified diff。
  - **🗄️ 备份 / 恢复**：每次保存前自动生成的备份列表（新的在前），一键恢复（恢复前会把当前文件再
    备份一份，恢复本身也可撤销）。
  - **➕ 新建 / 复制工作流**：空白工作流（带一个能通过校验的起始 step）或复制已有工作流（单文件模式
    共享 `prompt_file` 时会提示）；名称只允许字母 / 数字（含中文）/ 下划线 / 中划线。
...
对应 `next_doc/initiative_systems_unification_plan.md` 阶段一
（数据来自 `GET /v1/self/initiative_inbox`）。

把成长顾问 / 能力学习 / soft_goal_deriver 三条各自独立的候选队列聚合
成一个统一列表：

- 按 `domain` 多选筛选（🌱 用户成长 / 🎓 Agent 知识/人设 / 🧭 Agent 自身
  行为），每类候选数量用 `st.metric` 展示；
- 每张候选卡片展示标题、说明、置信度（如有）、生成时间，以及一句
  "→ 前往「XX」处理"的跳转提示；
- **这个 tab 本身不提供任何采纳/忽略/回答操作**——三路来源各自的
  写操作语义不同（成长顾问要走 `/growth adopt-goal`、能力学习问题要
  提交文本回答、soft_goal_deriver 的 Goal 要走目标看板的状态流转），
  勉强在聚合层统一写接口会丢失各自专属的校验/文案，因此定位明确是
  "先看清楚全貌"，真正处理请跳到卡片提示的原生 tab。
- `soft_goal_deriver` 一路只展示"从未被单独 touch 过"的 `agent_derived`
  Goal（`created_at`/`last_touched_at` 差值在 5 秒内），已经被用户或
  执行引擎推进过的不会重复出现在收件箱里。

后续阶段（阶段二起）会把"候选采纳"逐步接入统一的目标树执行通道，届时
这个 tab 会补上跳转即操作的能力；当前阶段刻意只做只读聚合，降低改动
风险。

> `GET /v1/self/personal_state`（用户当前处境快照）与 `GET
> /v1/self/priority_briefing`（今天最重要的事/AI已完成/需要你决定/风险
> 四段式简报，见 [Personal Model / State / Context Pack / Priority Briefing 指南](personal-model-context-pack-guide.md) §2、§4）
...
> 是两个不同的机制，命名曾经容易混淆（已改名为 Priority Briefing 避免
> 撞名），不要混用端点。
