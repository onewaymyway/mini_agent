...
- 目前已覆盖旧看板 18 个 Tab 中的：对话、会话管理、目标看板、工作流、文件浏览（通用能力）、
  产出物浏览+预览、自我状态、诊断信息、错误日志统计，共 9 个 Tab（部分为合并页面）。
  尚未迁移：成长顾问、能力学习、进化提案、Cron 任务、全局日程、
  外部输入网关、关注与通知、配置管理（表单编辑）、混合执行，共 9 个 Tab，
  期间可以继续使用旧版 `apps/mini_agent_kanban` 处理这些场景，详见方案文档第 5 节 P7~P11。
...
- 工作流（P6）内暂未覆盖：单步编辑 patch（修改已保存工作流定义本体，目前只做了
