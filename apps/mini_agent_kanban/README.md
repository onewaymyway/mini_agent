...
登录成功后会签发一个免登录 token，写进浏览器 Cookie（`mini_agent_kanban_auth`），
12 小时内刷新页面/重新打开标签页不用重新输密码。签名密钥存在
`.agent/kanban_session_secret`，删掉这个文件会让所有已签发的免登录 token
**全部**失效（相当于强制所有人重新登录）——这是"核选项"，日常场景更常用的是
下面的细粒度撤销。

> **[变更]** 这个 token 早期版本是拼在 URL 里的（`?auth=...`），2026-09 起改成存
> 浏览器 Cookie，不再出现在地址栏——原因是 URL 会被浏览器历史记录、反向代理/
> 服务器访问日志、Referer 头等途径记录下来，存在泄露面；升级到新版本后无需
> 任何手动迁移，旧版本签发的 `?auth=...` 链接会在下一次访问时被自动清理并要求
> 重新登录。需要额外安装依赖：`pip install extra-streamlit-components`（见
> `requirements.txt`），只有开启 `--require-login` 时才会用到。

**这个 token 本质上是一份免密登录凭证**：Cookie 依然是"页面自身 JS 能读到"的
明文 Cookie（不是 HttpOnly），不能防"页面被注入恶意脚本（XSS）后窃取 Cookie"
这类攻击；如果这台设备/浏览器被他人接触到（共享电脑、设备丢失、恶意浏览器
插件……），拿到它的人能直接登录进去，在 token 过期之前一直有效——所以请像
对待密码一样对待这台设备的登录状态，不使用的设备记得"退出登录"。
