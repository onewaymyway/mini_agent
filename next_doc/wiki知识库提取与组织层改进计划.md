...
| 编号 | 问题 | 所属层面 | 严重度 | 依赖关系 |
|---|---|---|---|---|
| E1 | 抽取时机与对话粒度错配（绑死在 compact 上） | 提取 | 高 | 无前置依赖，可独立做 |
| E2 | 抽取任务耦合度过高，四子任务挤在一次 LLM 调用 | 提取 | 中 | 无前置依赖，可独立做 |
| E3 | 抽取"看不到"已有知识库，每次从零识别实体 | 提取 | 高 | 依赖组织层 O1（索引可增量读取）打底 |
| O1 | 全量扫描架构没有为增长设计分层 | 组织 | 中 | 无前置依赖，且是 E3/O2/O3 的基础设施 |
| O2 | 实体关系图过于扁平，缺层级/路径概念 | 组织 | 中 | 依赖 O1（分层索引） |
| O3 | topic 聚类是纯事后归纳，不会动态更新已有页面 | 组织 | 中 | 依赖 O1 |
| O4 | decision/experience/entity/fact 四类页面缺统一知识生命周期状态机 | 组织 | 高 | 无前置依赖，但改动面最广，建议最后做 |
...
| 阶段 | 内容 | 预计工作量 | 前置依赖 |
|---|---|---|---|
| 第一批 | O1（索引复用 + 信度分层字段，不含分区） | 2-3 天 | 无 |
| 第一批 | E2 方案 B（schema 字段顺序调整 + 观测对比） | 0.5 天 | 无，可与 O1 并行 |
| 第二批 | E3（实体索引反向注入抽取 prompt，过渡版先不依赖 O1 排序） | 2-3 天 | 建议 O1 完成后做完整版，过渡版可提前 |
| 第二批 | E1（抽取时机解耦，独立触发器 + cursor 机制） | 3-4 天 | 无强依赖，可与 E3 并行 |
| 第三批 | E2 方案 C（compact 与独立抽取路径的开关切换） | 1 天 + 观测期 | 依赖 E1 完成并观测稳定 |
| 第三批 | O2（多跳衰减图扩展） | 2 天 | 依赖 O1 |
| 第三批 | O3（topic 再巩固） | 2 天 | 依赖 O1 |
| 第四批 | O4（统一知识生命周期状态机） | 3-4 天 | 依赖 O1-O3、E1-E3 均验证稳定 |
