...
> **状态**：第 5 节的**六个固定批次已全部完成**——第一批
> （4.1+4.5+4.8）、第二批（4.2+4.3+4.6）、第三批（4.4+4.13）、
> 第四批（4.9+4.11）、第五批（4.10）、第六批（4.7），详见
> `PROJECT.md`"阶段三十三第一批~第六批"条目。**4.12（引擎与 LLM
> 分工的渐进式重构）三步现已全部补做完成**——第一步"校验前移"
> （`world_simulator/decision_validation.py`）、第二步"节点状态推导
> 前移到引擎"（`causal_tree.suggest_status_transitions()`），详见
> `PROJECT.md`"阶段三十三第七批"条目；**第三步"世界演化/决策生成
> 拆成两次调用"已于阶段三十三第八批实现**——应用户明确要求"不再
> 等待触发条件观察，直接实施"，新增 `manifest.settings.
> split_decision_calls` 开关（默认 `False`，向后兼容），为 `True`
> 时 `engine.advance()` 改为调用 `workflows/world_evolve.yaml`
> （只产出世界状态变化）+ `workflows/decision_generate.yaml`（专门
> 产出候选选项）两次独立 workflow，详见 `PROJECT.md`"阶段三十三第
> 八批"条目，全部测试通过（**339 passed**）。至此，第 3 节列出的
> 13 条差距（4.1~4.13）全部落地完成，方案范围内没有遗留项。
...
3. **第三步（尝试拆分"世界演化"与"决策生成"为两次调用）**：这是
   ...
   **实际落地情况（阶段三十三第八批，应用户明确要求"不再等待触发
   条件观察，直接实施"）**：拆成两次 LLM 调用确实带来了本节原本预计
   的成本和延迟翻倍，落地时的做法是新建两个独立的单步 workflow
   （`workflows/world_evolve.yaml`/`workflows/decision_generate.yaml`），
   `world_evolve` 产出 `next_vars`/`narrative`/`tree_updates` 等世界
   状态字段，其完整输出通过 `evolved_summary`/`evolved_narrative`/
   `evolved_vars_json`/`evolved_time_label`/`evolved_time_granularity`/
   `evolved_key_drivers_json`/`evolved_tree_updates_json` 这几个
   `decision_generate` 专属输入原样喂给第二次调用（对应本段原本
   设想的"把第一次的完整输出原样传给第二次"）；`engine.advance()`
   新增 `manifest.settings.split_decision_calls` 开关（默认
   `False`，沿用原单次调用 `advance_step.yaml`，向后兼容），为
   `True` 时才走两次调用路径，是否承受这个成本由用户自己决定，不是
   新的默认行为。**没有做的部分**：不做"根据实测效果自动切换"这种
   动态判断（本节原文设想的"触发条件"完全交给用户自己判断要不要
   开启这个开关，不是代码自动侦测"选项质量是否不达预期"后自动切换
   ——那需要一套质量评估机制，属于比这条改动本身更大的新课题，本轮
   不展开）。
- **验收点**：`decision_validation.py` 模块的单元测试（校验规则
  逐条覆盖）；`suggest_status_transitions()` 的单元测试（构造
  超过 `time_window` 仍是 `dormant` 的节点，断言给出"建议
  expired"的提示文本）；第三步（阶段三十三第八批）新增
  `tests/test_split_decision_calls.py`（拆分模式端到端验证
  `world_evolve`→`decision_generate` 的调用顺序、字段合并、
  `decision_opportunity` 正确构造；默认模式不触碰新 workflow 的
  回归验证）。
...
7. **第十二条（4.12）跨越多个批次**：第一步（校验前移到
   ...
   以上两步已在阶段三十三第七批完成；第三步（两次调用拆分）原本
   按 4.12 节所述是带触发条件的观察项，应用户明确要求已在阶段
   三十三第八批直接实施（不再等待触发条件），至此第 5 节六个固定
   批次和 4.12 三个子步骤全部完成。
