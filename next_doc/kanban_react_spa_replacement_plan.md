...
> **⚠️ 当前状态（2026-08 更新）：`mini_agent_kanban_x` 是实验/试点项目，尚未成为日常
> 使用的看板。`apps/mini_agent_kanban`（Streamlit）才是当前真正在用的看板。**
> **今后的看板相关改动（bug 修复、功能调整）一律只改 Streamlit 一侧，除非用户明确
> 要求同步到 React 看板。** 本文档记录的 P0~P11 是这次技术验证过程的历史记录，
> 不代表"已经切换到新看板"——是否/何时正式切换取决于后续验证结果，另行决定。
...
- 2026-08-17（第六次交付，本次）：完成 P7 成长顾问 + 能力学习。**成长顾问**：诊断信息面板
  ...
  **暂未覆盖**（留待后续，优先级低于核心信息展示/审批闭环）：候选看板拖拽式分栏、按主题采纳/
  ...
  用户管理、生产收尾）仍按原计划待迭代。
- 2026-08-17（第七次交付，本次）：完成 P8：进化提案 + Cron 任务 + 全局日程。
  **进化提案**：提案列表（risk 徽标）、diff 查看（按文件分组，单文件默认展开）、合并
  （risk=low 一键合并，非 low 需二次确认强制合并）。**Cron 任务**：新建任务（名称/表达式/
  Prompt/描述/优先级）、任务列表（启用禁用、优先级内联调整、立即运行、提意见反馈、删除，
  系统内置任务不可删除）、任务工作区抽屉（state 摘要、专属 Prompt 查看编辑、最近执行记录+
  成功失败角标、单次执行事件展开、卡死重置）。**全局日程**：未来到期 cron 任务列表、周期性
  Goal 下次触发列表、调度仲裁状态变化时间线（含占比摘要）、调度公平性诊断只读展示（含逐
  objective 明细）。`npm run build` 验证通过。
  P9~P11（外部输入网关、关注通知、混合执行、配置管理表单、用户管理、生产收尾）仍按原
  计划待迭代。
- 2026-08-17（第八次交付，本次）：完成 P9：外部输入网关 + 关注与通知 + 混合执行。
  **外部输入网关**：已注册来源列表（类型/启用/轮询间隔/运行中/上次轮询/连续失败熔断/
  最近错误）、热重载 sources.yaml、路由规则只读展示（policies.yaml）、待处理告警列表、
  新颖信号候选（确认为目标/忽略）、最近事件流水、外部知识反馈闭环 P1~P5 五个子面板
  （候选队列过期巡检/wiki利用率/阈值自校准/外部趋势×能力薄弱点/生态定位扫描/月度战略
  回顾）。**关注与通知**：关注对象列表（watchlist.yaml）、分级汇报规则（report_tiers.yaml，
  含下次触发/空转计数）、待处理汇报列表（展开详情+标记已读）、通知发送记录。**混合执行**：
  按 task_id 分组的折叠面板，展示脚本仓库状态（当前版本/版本总数/创建者/成功失败次数/
  连续失败）+ run 统计汇总 JSON。`npm run build` 验证通过。**暂未覆盖**（留待后续，优先级
  较低）：来源健康趋势图（`health_history` 端点已接入但暂未在页面上可视化，只做了数据层
  hook）、归档查询面板（`archive/query`，仅接入 endpoint，未做查询表单 UI）。同时修复了
  打包过程中再次遗漏的 `pages/Sessions` 页面文件（`npm run build` 报 module not found，
  已重建：会话列表/新建/恢复/删除/详情抽屉，与 `useSessions.ts` 现有 hook 对齐）。
  P10~P11（配置管理表单、用户管理、账户登录门禁完整版、生产收尾）仍按原计划待迭代。
- 2026-08-17（第九次交付，本次）：完成 P10：配置管理 + 用户管理 + 账户登录门禁核对。
  **配置管理**：按分类折叠展示 `agent_config.json` 全部已收录字段（`GET /v1/self/config`），
  支持关键字过滤（字段名/中文说明）、"已自定义"徽标、按字段类型（bool/int/float/str）
  渲染对应控件（Switch/InputNumber/Input），敏感字段只展示"已配置/未配置"不可编辑；
  本地暂存修改（"未保存"徽标）后批量提交 `PATCH /v1/self/config`，保存成功提示"部分配置
  需要重启 agent 进程才能生效"。复用了 P4 阶段已声明但未接入 UI 的
  `getSelfConfig`/`patchSelfConfig` 端点封装（之前只在 `useSelfStatus.ts` 里定义、未被任何
  页面渲染），本次补齐为独立的 `Config` 页面。**用户管理**：用户列表（ID/名称/角色/信任
  等级/创建时间/最近活跃）、新建用户（弹窗表单，创建成功后一次性展示明文 Token 并支持
  一键复制）、修改角色（内联下拉，owner 角色禁止在此修改）、重置 Token（同样一次性展示）、
  删除用户；非多用户模式（daemon 未加 `--http-multi-user` 启动）时识别 404/提示信息，展示
  "多用户模式未开启"提示而不是报错空白页。**账户登录门禁**：核对后端目前只有 Bearer Token
  鉴权（`Authorization: Bearer <token>`），没有独立的用户名+密码登录/换取 Token 的 REST
