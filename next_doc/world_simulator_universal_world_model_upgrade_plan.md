...
> 十四（4.6 节，Problem Compiler 进阶：目标驱动的自动排序）均已完成，
> 见 `PROJECT.md` 对应交付记录。演进计划 4.1~4.6 节至此全部落地。
> 阶段十五~十八（4.7~4.11 节，含一个不排期项）是对原第 6 节"本次不做
...
字段）已经落地，按文档顺序在它们之后完成，至此演进计划 4.6 节
也已落地。

4.7~4.11 节（阶段十五~十八，含一个不排期项）是对"范围收窄"部分
（4.5 节遗留的完整因果链）和原"本次不做的事"清单（资源关系建模、
Belief/图结构、Hierarchical Agent、Reality Sync）的后续可执行拆解：
阶段十五、十六是"已有地基、建议先收集反馈再启动"的计划中项目；
阶段十七、十八和 Hierarchical Agent 是"条件触发"项目——设计草案
已经写好，但明确要求出现对应的真实场景才启动，避免在没有验证需求
的情况下过度设计。

## 6. 尚未启动项目的汇总（原"本次不做的事"，已拆解为可执行计划）

以下条目此前只是一句话式地记录"暂不做"，现已分别拆解为 4.7~4.11 节
