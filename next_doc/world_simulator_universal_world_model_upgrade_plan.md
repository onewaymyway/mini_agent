...
> 阶段十四~十九（4.6~4.11 节）是对"范围收窄"部分与原第 6 节"本次不做
...
### 4.6（P1，计划中，阶段十四）Problem Compiler 进阶：目标驱动的自动排序
...
### 4.7（P2，计划中，阶段十五）Evidence Chain 进阶：结构化因果链
...
- **阶段十四**（计划中）：本文档 4.6 节，Problem Compiler 进阶——
  目标驱动的自动排序（依赖阶段十、十二，建议先收集使用反馈再启动）。
- **阶段十五**（计划中）：本文档 4.7 节，Evidence Chain 进阶——结构化
  因果链（依赖阶段十三，建议先收集使用反馈再启动）。
...
按文档顺序排在最后完成。至此演进计划 4.1~4.5 节全部落地。

4.6~4.11 节（阶段十四~十八，含一个不排期项）是对"范围收窄"部分
（4.4/4.5 节遗留的自动排序、完整因果链）和原"本次不做的事"清单
（资源关系建模、Belief/图结构、Hierarchical Agent、Reality Sync）
的后续可执行拆解：阶段十四~十六是"已有地基、建议先收集反馈再启动"
...
以下条目此前只是一句话式地记录"暂不做"，现已分别拆解为 4.6~4.11 节
的具体计划（含问题/方案/涉及文件/验收标准/优先级与依赖），本节只做
索引，不重复内容：

| 原条目 | 对应计划 | 状态 |
| --- | --- | --- |
| Problem Compiler 自动排序（阶段十二范围收窄部分） | 4.6 节，阶段十四 | 计划中，建议先收集使用反馈 |
| Evidence Chain / Debug Trace 完整版 | 4.7 节，阶段十五 | 计划中，建议先收集使用反馈 |
