...
> Entity/Relationship 图结构）、阶段十八（4.11 节，Reality Sync 轻量
> 版）、阶段十九（4.10 节，Hierarchical Agent 设计草案第一步）**均
> 已完成，但都属于"未等待触发条件、提前实施"的特殊情况**，见各节
> "实施记录"里的完整说明——不是常规的按需求驱动落地，记录在案供以后
> 复盘。至此演进计划 4.1~4.11 节全部落地，但 4.9~4.10 节的完成方式与
> 其它各节不同，不应被当成"条件触发原则不再适用"的先例。
...
### 4.10（已完成，阶段十九）Hierarchical Agent / Dynamic Cognition Router——设计草案第一步
...
**为什么原本不排期**：`world_simulator` 当前的应用场景边界很明确——
...
**触发条件（原定，实际未满足）**：出现真实需求，要求单次模拟包含
"数量级明显超过个位数的独立决策主体"（比如"模拟一个 50 人的部落，
每个人有独立目标"），且这类场景验证了 4.9 节的"少量多主体"方案
（`entities` 字典）在性能/成本上明显撑不住（比如每步都要对 50 个
entity 分别给出决策，单次 `advance_step` 调用的输出体积/推理时间已
经不可接受）。

**设计草案（第一步，不是最终方案）**：把 entity 分成"关键角色"
（需要完整 LLM 推理，走现有 `advance_step` 流程）和"背景角色"（用
规则/简化逻辑批量更新，不调用 LLM，比如按预设的简单趋势外推
`vars`），而不是一开始就设计一套通用的"认知路由器"。这样即使只做了
这一步也有独立价值（背景角色确实不需要每步都调用一次 LLM），且不
需要预判"路由策略"这类目前完全没有真实场景验证过的抽象。

**实施记录**：**本节触发条件实际并未满足**——项目里目前唯一用到
`multi_entity_mode` 的场景是"多方谈判"（阶段十七），主体数量最多
两三个，远没到"数量级明显超过个位数、验证了 4.9 节方案撑不住"的地步。
本节是应用户在没有具体场景的前提下明确要求"把剩下的条件触发项也提前
实施成代码"而完成的，执行前已经明确提醒过这个偏离（并且这已经是这
一轮对话里第二次做出这个提醒），记录在案供以后复盘。

按"设计草案第一步"实现，**没有实现"调度框架"/"认知路由器"**：新增
`SimManifest.settings.hierarchical_agent_mode`（默认 `False`）+
`background_entities`（背景角色名字列表，需要与 `vars.entities` 的键
一致，只有 `multi_entity_mode` 同时启用时才有意义）；`engine.py` 新增
`_apply_background_entity_extrapolation()`，`advance()` 落盘前对声明
的每个背景角色，用它"上一步到这一步"每个数值字段的变化量按相同量再
外推一步，**强制覆盖** LLM 这一步给这些主体的输出（不管 LLM 实际给了
什么值，不"轻信"、不做"部分采纳"）；没有"上一步"可参考（比如第一次
推进）或字段非数值时，外推量按 0 处理，即原样保留当前值，不是凭空
编一个趋势。`spec_generator.resolve_hints()` 新增
`background_entities_hint`，提示 skill "这些主体不用深入推理，反正
不会被采纳"，减少不必要的推理消耗（但**没有把背景角色从 prompt 输入/
输出里物理剔除**，`advance_step` 仍然是一次包含全部主体的 LLM 调用，
这是与"完整调度框架"最大的差距，也是这次没有解决"成倍调用成本"问题
的根本原因——真正的成本节省需要动态拼装 prompt，排除背景角色的输入
文本，这里没有做）。`app.py` 新增背景角色声明的折叠区（创建向导 +
详情页"模拟设置"），时间线（含游戏化视图）新增"🧩 背景角色由规则
自动外推"的信息性提示（新增 CSS `ws-chapter-background-entity-note`，
措辞刻意区别于两种资源相关提示——这不是"问题"，是设计使然）。
`SimState.background_entities_applied` 记录每一步实际生效外推的
主体名字，供展示层使用。

验收覆盖：`tests/test_spec_and_engine.py` 新增 4 个用例——第一次推进
无"上一步"时外推量为 0（强制覆盖 LLM 的编造值）、有真实历史趋势时
按线性关系正确外推（手动构造跨两步的真实数值变化验证）、
`hierarchical_agent_mode` 关闭时完全不介入（向后兼容）、
`resolve_hints` 三种提示文案（未启用/启用但未列名字/启用且列出
名字）。累计 106 个测试全部通过。

**已知限制（如实记录）**：这是"分层"的第一步，不是完整的"认知路由
器"——没有"路由策略"、没有按场景动态决定哪些主体该升级成"关键角色"，
也没有真正做到"背景角色不消耗 LLM 推理成本"（仍在同一次调用里，只是
输出不被采纳，理论上还是浪费了一点 token；见上一段说明）。**未经过
任何真实的"大量主体"场景验证**，线性趋势外推是否是合适的简化模型、
`background_entities` 该怎么声明才顺手，都只是这次的一次性设计判断，
风险与阶段十七相当（甚至更高，因为连"设计草案本身有没有真实价值"都
没有验证过）。`hierarchical_agent_mode` 未声明或为 `False` 时，行为
与引入这个功能之前完全一致，不影响其它任何模板/场景。

### 4.11（已完成，阶段十八）Reality Sync 轻量版：外部数据手动校准输入
...
**实施记录**：**本节触发条件实际并未满足**——没有任何真实使用场景
明确要求"贴合具体真实数据"，本节与 4.10 节（阶段十九）一起，是应
用户在没有具体场景的前提下明确要求"把剩下的条件触发项也提前实施成
代码"而完成的，记录在案供以后复盘。

按上述方案原样实现：`SimManifest.settings.calibration_notes`（默认
空字符串）；`spec_generator.generate_scenario()` 与 `engine.advance()`
都把这个字段原样传入对应 workflow 的 `calibration_notes` prompt
输入（`generate_scenario.yaml`/`advance_step.yaml` 新增
`{calibration_notes}` 占位符，为空时提示语明确写"说明没有需要参考的
真实数据"，不会出现空白突兀的占位）；`app.py` 创建向导与详情页
"模拟设置"区块都新增"填入真实世界参考信息"折叠区（文本框）。**没有
做任何自动数据抓取/更新**，是否采信、怎么融入完全由 LLM 判断，与
方案原文一致。

验收覆盖：`tests/test_spec_and_engine.py` 新增用例
`test_advance_passes_calibration_notes_to_prompt_inputs`，用
monkeypatch 打桩验证 `calibration_notes` 原样出现在
`advance_step` 调用的 `inputs` 字典里（`generate_scenario` 一侧的
传递逻辑与之完全对称，未单独重复打桩验证，风险很低）。累计 106 个
测试全部通过。**未接入真实 LLM 验证"是否真的被采信"**——按方案原文
的说明，这属于 prompt 质量调优范畴，不是这次要验收的内容。这是本节
三个"提前实施"项目（阶段十七/十八/十九）里改动范围最小、风险最低的
一个——纯粹的"原样透传一段文本"，没有新增任何校验/覆盖逻辑，不存在
"设计判断可能错"的风险。
...
- **阶段十八**（已完成，但提前实施）：本文档 4.11 节，Reality
  Sync 轻量版——外部数据手动校准输入，触发条件（出现真实的"希望
  模拟贴合具体真实数据"需求）**在实施时并未满足**，同样是应用户
  明确要求提前实施，见该节"实施记录"。
- **阶段十九**（已完成，但提前实施）：本文档 4.10 节，Hierarchical
  Agent / Dynamic Cognition Router 设计草案第一步——触发条件（出现
  "数量级明显超过个位数的独立决策主体"场景，且验证阶段十七的方案
  撑不住）**在实施时并未满足**，只做了"分层"本身（关键角色/背景
  角色），没有做"调度框架"，同样是应用户明确要求提前实施，见该节
  "实施记录"，此前"不排期，无阶段编号"的记法已随实施完成更新为
  正式阶段编号。
...
4.8~4.11 节（阶段十六~十九）是对原"本次不做的事"清单（资源关系
建模、Belief/图结构、Hierarchical Agent、Reality Sync）的后续可
执行拆解：阶段十六（资源转移关系建模）已完成，验收标准见 4.8 节
"实施记录"，是常规的"符合触发条件后实施"；阶段十七（Belief/图
结构）、阶段十八（Reality Sync 轻量版）、阶段十九（Hierarchical
Agent 设计草案第一步）也都已完成，但**均是在各自触发条件未满足的
情况下被提前实施的**，见各节"实施记录"里的完整说明与风险提示——
这与阶段十六的完成方式不同，不应被当成"条件触发原则已经不再适用"
的先例：这三节的"条件触发"表述本身没有错，只是这次被用户明确要求
跳过了，以后遇到类似的"没有真实场景、要不要先做"的判断，仍然应该
按"条件触发"原则先确认，而不是默认可以跳过。
...
| Hierarchical Agent / Dynamic Cognition Router | 4.10 节，阶段十九 | 已完成设计草案第一步（非完整调度框架），但属于提前实施，未经真实场景验证，见该节"实施记录" |
| Reality Sync | 4.11 节，阶段十八（轻量版） | 已完成轻量版，但属于提前实施，见该节"实施记录"；完整版（自动数据源接入）仍不在计划内，若以后真的需要，需另开文档 |
