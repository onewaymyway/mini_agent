...
> 批次三（问题图可视化）已实施完成，详见本文档"实施记录"一节及
> `PROJECT.md` 对应变更记录；批次三里"可选加强"的信号触发扫描节奏
> 按方案原文建议暂不做（需要先观察批次一固定间隔在实际使用中的
> 效果），不是遗漏。本文档合并两个来源的问题：
...
## 4.3 实施记录：批次三（问题图可视化）已完成

**完成时间**：2026-09-21。**范围**：与本文档第 3 节"批次三"设计
中"问题图可视化"部分一致；"可选加强"的信号触发扫描节奏**按方案
原文自己的建议**暂不做——原文明确写着"不建议在批次一验证之前就
直接做这一层，避免同时改两个变量导致'扫描到底有没有用'这个问题
更难判断清楚"，本轮尚未观察到批次一在真实使用中的效果，所以维持
"不做"这个默认状态，不是本轮遗漏。

- `app.py` 新增 `_collect_problem_graph_nodes()`/`_problem_graph_
  edges_to_dot()`/`_render_problem_graph_section()` 三个函数，写法
  完全复用 `_causal_graph_edges_to_dot()`（第八轮批次四）已经验证
  过的"纯函数转 DOT + `st.graphviz_chart()` 渲染"思路，没有引入
  任何新的可视化库/前端依赖。
- 没有 `id` 的问题条目处理：批次二里 `depends_on` 声明的可能是
  真实 `id` 也可能是自由文本描述（`suggest_problems()` 建议阶段还
  不知道最终会落到哪个 `id`），所以图渲染层选择"不做去重、不做
  存在性校验"，让这类引用在图上表现为孤立节点或悬空边——这是刻意
  的，不是 bug，同 `depends_on` 字段本身"提示而非强制"的定位一致。

详细改动清单见 `PROJECT.md` "第十轮批次三" 条目；测试见新文件
`tests/test_problem_graph_visual.py`（11 个用例）。

**后续节奏**：第十轮问题清单的三个批次到此全部完成。是否要做"信号
触发的扫描节奏"这个可选加强项，建议等 `problem_discovery_auto_
scan_interval` 在真实 life_sim 实例上跑过一段时间、观察固定间隔是否
已经够用之后再决定，不建议现在就做。
