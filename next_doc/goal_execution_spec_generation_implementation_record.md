...
- `_compute_spec_diff(old_spec, new_spec) -> dict`：对
  ...
  criteria`/`overall_completion_criteria` 五个列表字段，各自用一个
  "识别 key"（分别是 `name`/`key`/`name`/`text`/`text`）把新旧两份
  草稿的条目配对，算出 `added`（新版独有）/`removed`（旧版独有）/
  `changed`（key 相同但内容不同）三类；`special_constraints` 是纯
  字符串列表，没有"识别 key"，按值本身直接做集合差（同一条要么原样
  保留、不出现在结果里，要么被视为"删旧增新"两条，不细分"改写"）。
  只有真的存在差异的 section 才会出现在返回的 dict 里，内容完全一致
  的 section 不出现——调用方据此判断"这次到底改了什么"。
- `_render_spec_diff(diff)`：把上面的结果渲染成"➕ 新增（绿色）/
...
- 新增 `diff_key` session_state 键：「🔄 补充意见重新生成」/「📄 从模板
  ...
  草稿存进 `diff_key`。
- 渲染草稿摘要之后，如果 `diff_key` 存在，展开一个"🔍 与上一版的差异"
  区块（默认展开），对比 `diff_key` 存的旧版本与当前 `draft_key` 算出
  差异并展示；区块内有「知道了，收起差异」按钮，点击后清空 `diff_key`
  （不会自动过期，用户想留着对照多看几眼也可以）。
- 「✅ 确认使用此规范」「❌ 放弃草稿」「♻️ 生成新草稿（重新想一遍
  细节）」三处收尾动作都会顺带清空 `diff_key`——避免下次打开这个 Goal
  的规范面板时莫名其妙冒出一份"上上次"的差异对比。
...
- 方案 §6.2 原话是"新增条目、删除条目、被改写的条目分别标出来"，
  ...
  的行内 diff，比如某条 `per_cycle_criteria.text` 从"报告要写得详细"
  改成"报告文件里必须出现'环比'字样"，只标"✏️ 改写：报告文件里必须
  出现'环比'字样"，不额外展示旧文本/做字符级高亮）——多数 Goal 执行
  规范的字段是短句/文件名，行内 diff 的收益相对有限，改写后的完整
  新内容已经足够看出变化，暂不做更细粒度的字符级对比。
- 仍然没有实现方案 §6.1/§6.2 描述的"直接编辑字段文本框"（按行拆分手工
  微调某个字段的具体文字）——差异高亮是"反馈驱动迭代"这条主线上更
...
正确检测；`per_cycle_criteria`/`overall_completion_criteria` 按 `text`
匹配；`special_constraints` 纯字符串列表的加减检测（不产生 `changed`）；
未变化的 section 不出现在结果里（对应字段级锁定场景）；spec dict 缺失
某个 key 时按空列表兜底、不抛异常。全部通过直接导入 `apps/mini_agent_
kanban/app.py::_compute_spec_diff`（不依赖真实 Streamlit 运行时上下文，
`_render_spec_diff()` 的 UI 渲染部分因依赖 Streamlit 脚本上下文不在
单元测试里覆盖）。
