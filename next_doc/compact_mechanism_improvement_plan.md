...
> **后续修复（2026-09-14）：** 本计划的 P0/P1/P2 各项都假定 `run_turn(compact_prompt)`
> 正常路径要么返回真实摘要文本，要么抛异常——但实际还存在第三种情况：
> `run_turn()` 内部结果健全性校验命中畸形输出时会返回一段**非空**的固定
> 哨兵占位文本而不抛异常。这个情况当时未被本计划覆盖，导致一次真实故障
> （哨兵文本被当真实摘要写入历史，原始历史丢失）。修复方案与验证见
> `next_doc/compact_result_validity_guard_plan.md`，代码落在 P0 同一个
> `compact_with_skills()` 正常路径上，逻辑上可视为对本计划 P0 的补丁。
