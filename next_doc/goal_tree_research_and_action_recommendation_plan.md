...
5. **阶段五（报告立即生成 + 存储位置改造，用户反馈驱动）**：见
   `goal_tree_research_report_visibility_plan.md`——阶段二遗留的
   "🔍 立即调研只生成候选、不生成报告"被用户反馈为"看不到调研报告"，
   连带把报告存储位置从全局平铺的 `wiki/growth/` 改成节点专属的
   `goals/<node_id>/research/`，并在看板"📄 相关调研"区块补上报告
   查看入口（列出该节点全部调研历史，不限 pending）。
