...
> 本计划是对 `next_doc/wiki-style-knowledge-base-improvement-plan.md`（P0-P4）的**后续深化**，不重复其已完成的工作，
...
|---|---|---|---|---|---|
| E1 | 抽取时机与对话粒度错配（绑死在 compact 上） | 提取 | 高 | 无前置依赖，可独立做 | **已完成** |
| E2 | 抽取任务耦合度过高，四子任务挤在一次 LLM 调用 | 提取 | 中 | 无前置依赖，可独立做 | 方案B已完成；方案A随E1完成自然解决；方案C机制已就位，切换本身是观察期后的人工操作（见E1记录§3） |
...
> **实施状态：§1.2.1/§1.2.2 已完成**，详见 `next_doc/wiki-extraction-layer-plan-e1-record.md`
...
> **实施状态：方案B 已完成**，详见 `next_doc/wiki-extraction-layer-plan-e2-planb-record.md`
...
> **实施状态：已完成**，详见 `next_doc/wiki-extraction-layer-plan-e3-record.md`
...
> **实施状态：§4.2.1 / §4.2.2 已完成**，详见 `next_doc/wiki-extraction-layer-plan-o1-record.md`
...
> **实施状态：已完成**，详见 `next_doc/wiki-extraction-layer-plan-o2-record.md`
> （`GraphIndex.expand()` 多跳衰减扩展 + `expand_legacy()` 保留一跳兼容行为 +
> `search.py` 自动/`--deep` 强制深度检索 + LLM 精排 prompt 权重标注 + 单测）。

### 5.2 改进方案 `[设计]`（已完成，见上方实施状态）
...
> **实施状态：已完成**，详见 `next_doc/wiki-extraction-layer-plan-o3-record.md`
> （`_find_topic_reconsolidation_candidates` 重合度扫描 + `append_to_topic_page`
> 追加并入 + `writer.append_section()` 扩展支持链接/frontmatter 同步更新 +
> `reconsolidation_interval_runs` 频率控制 + `topics_run_counter.json`/
> `topics_reconsolidation_log.jsonl` 观测基础设施 + 单测）。

### 6.2 改进方案 `[设计]`（已完成，见上方实施状态）
...
> **实施状态：已完成**，详见 `next_doc/wiki-extraction-layer-plan-o4-record.md`
...
| 阶段 | 内容 | 预计工作量 | 前置依赖 | 状态 |
|---|---|---|---|---|
| 第一批 | O1（索引复用 + 信度分层字段，不含分区） | 2-3 天 | 无 | **已完成**（`wiki-extraction-layer-plan-o1-record.md`） |
| 第一批 | E2 方案 B（schema 字段顺序调整 + 观测对比） | 0.5 天 | 无，可与 O1 并行 | **已完成**（`wiki-extraction-layer-plan-e2-planb-record.md`） |
| 第二批 | E3（实体索引反向注入抽取 prompt，直接实现完整版，O1 已就绪） | 2-3 天 | 建议 O1 完成后做完整版，过渡版可提前 | **已完成**（`wiki-extraction-layer-plan-e3-record.md`） |
| 第二批 | E1（抽取时机解耦，独立触发器 + cursor 机制） | 3-4 天 | 无强依赖，可与 E3 并行 | **已完成**（`wiki-extraction-layer-plan-e1-record.md`） |
| 第三批 | E2 方案 C（compact 与独立抽取路径的开关切换） | 1 天 + 观测期 | 依赖 E1 完成并观测稳定 | 机制已就位（`extract_decisions`/`extract_world_model` 开关），切换本身待观测期后人工执行 |
| 第三批 | O2（多跳衰减图扩展） | 2 天 | 依赖 O1 | **已完成**（`wiki-extraction-layer-plan-o2-record.md`） |
| 第三批 | O3（topic 再巩固） | 2 天 | 依赖 O1 | **已完成**（`wiki-extraction-layer-plan-o3-record.md`） |
| 第四批 | O4（统一知识生命周期状态机） | 3-4 天 | 依赖 O1-O3、E1-E3 均验证稳定 | **已完成**（`wiki-extraction-layer-plan-o4-record.md`） |
