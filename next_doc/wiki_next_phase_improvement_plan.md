...
- §1.2.3「`check_and_plan()` 挂在 `sys:consolidation` 触发后顺带跑一次」尚未接入
  `evolution/autonomous_loop.py`——目前 `wiki/decommission.py::check_and_plan()`/
  `check_ready_transition()` 是独立可调用的函数，但还没有自动挂载到 daemon 循环里，
  需要人工或后续接入点调用（比如在 `/wiki promotion` 命令里顺带展示）。
...
- 本轮验证方式：新增代码逐一 `ast.parse` 语法检查通过；与改动直接相关的 9 个既有
  测试文件（125 个用例）全部通过；全量测试套件因单次执行时间超限未能跑完，但抽样
  比对显示失败用例在**修改前的原始代码上同样失败**（逐字符核对一致的 F/E 分布），
  判断为环境相关的既有失败（很可能依赖真实 LLM/网络），非本轮改动引入。
