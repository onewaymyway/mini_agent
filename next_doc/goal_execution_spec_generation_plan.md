...
- **状态**：Stage 1（核心数据模型/存储/生成器/模板库/配置 + 两处消费方
  ...
  展示，不需要额外 LLM 调用）+ Stage 12（`mode="auto"` 补上"LLM 自报
  `needs_project_context` 后二次重生成"兜底：规则粗筛没命中时先跑一次
  裸 LLM，若其自报需要项目上下文则丢弃结果、改用受限 Agent 路径重新
  生成一次，与老代码 `GoalSpecBuilder` 的三态设计完全对称）已实施。
  看板侧的"直接编辑字段文本框"仍未实施，见
