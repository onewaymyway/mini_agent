...
### P3 —— tick() 执行看门狗（从"暴露观测字段"升级为"主动检测告警"）
...
- **改动**：
  ...
  - 看板新增一个"🕹️ 统一调度总览"折叠区块，作为独立 tab 或并入现有
    "🩺 自诊断信号闭环"，具体信息架构留待 §3 待讨论问题确认。
- **验收标准**：
...
- **分步迁移路径（不是一次性重写）**：
  ...
  4. **接管实际派发**：三条通道的 `tick()` 触发点最终都收敛成
     ...
     `objective_executor` 相关方法的代码逐步收敛进去。
...
3. P4 的"统一调度总览"信息架构：独立新 tab，还是并入现有"🩺 自诊断信号
   闭环"区块，需要结合看板整体信息密度评估，不在本文档单独展开设计。
