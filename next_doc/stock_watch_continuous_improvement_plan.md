...
- 2026-08-27（续）：用户实测反馈——账本能看到详情后，定位到 `screener`
  ...
  - **未在真实网络下验证**：本次改造所在环境无法访问
    ...
    401，需要回来换方案
- 2026-08-27（续2）：用户在真实环境实测，"首页预热拿 hexin-v" 的假设
  被证伪——预热请求完成后 `session.cookies` 里根本没有 `hexin-v`，
  说明它不是服务端 `Set-Cookie` 下发的，而是前端加载一段混淆 JS
  （`hexin-v.bundle.js`）在浏览器里动态算出来的令牌（公开的逆向分析
  文章证实了这一点）。这里没有去逆向还原那段混淆 JS 的加密算法——
  那本质上是实现"绕过对方网站技术访问控制"，不该做。改为两条正路
  的分层策略，`data_sources.py::_fetch_iwencai_web()`：
  1. 检测到已安装可选依赖 `pywencai`（社区维护、内部用 Node.js 跑
     通那段 JS）→ 优先用它，令牌计算完全交给这个库维护
  2. 没装 `pywencai` 但配了环境变量 `STOCK_WATCH_IWENCAI_COOKIE`
     （用户从自己浏览器开发者工具复制的当前有效 `hexin-v` 值）→ 用
     这个手动令牌发请求；会过期，需要用户自己定期更新，代码没有
     自动刷新能力
  3. 都没有 → 退化到"首页预热"兜底路径（万一对方哪天把验证降级回
     简单 Set-Cookie 形式，不需要改代码就能重新工作），失败时的
     `DataSourceError` 消息直接指向上面两条真正可行的路，不再是一句
     不知道该怎么办的"401"
  用 mock 验证了三条路径：① 手动 cookie 失效时给出"令牌看起来已失效，
  需要重新复制"的针对性提示、② 无任何配置时给出"装 pywencai 或配
  环境变量"的引导提示、③ mock 了一个假的 `pywencai` 模块验证优先
  走它这条路径能正确返回结果。`stock_watch/tests/` 43 个既有测试
...
- 2026-08-27（续6）：**使用者跑了真实 Chrome 后反馈的 bug**——"执行
  完成，returncode=0，但实际有问题，打开调试浏览器之后，根本没有等待
  用户登录，就关闭了，然后获取了没用的 cookie"，也印证了上一条"没有在
  真实 Chrome 下验证"的担心是对的：假 CDP 服务器验证里"前两次轮询返回
  空 cookie、第三次才返回带值的 cookie"这个模拟本身就跟真实情况不符——
  `stock_watch/data_sources.py` 早就写明 `v` cookie 是页面加载时那段
  混淆 JS 无条件算出来的，真实 Chrome 里页面一打开、**第一次**轮询就
  会读到值，不会有"空几次再出现"这个过程；旧的判断逻辑（cookie 存在
  即完成）在假服务器的模拟下"看起来"跑通了，但假服务器模拟错了它本该
  验证的那个关键行为，掩盖了这个设计缺陷。修复详见
  `next_doc/external_projects_kanban_integration_plan.md`
  "2026-08-27：第三个使用者实测反馈的 bug"条目（改成交互式回车确认 +
  非交互式"等待值变化"两条路径）。**给验证方法本身补的教训**：给"轮询
  等待某个外部信号出现"这类逻辑写 mock/假服务器验证时，mock 的行为
  假设要跟被测对象自己文档里写明的真实行为对齐（这里 `v` cookie
  "无条件存在"这一点在 `data_sources.py` 里其实已经写清楚了），不能
  凭直觉编一个"合理但不真实"的模拟序列，否则验证跑通了也不代表设计
  是对的。
