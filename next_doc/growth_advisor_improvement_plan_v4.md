...
- **版本**: v1（草案，方向级 + 关键设计点级规划）——**N1 已实施**，详见
  `next_doc/growth_advisor_implementation_record.md` "N1：诊断面板健康度
  趋势化" 一节；N2-N5 尚未实施。
- **前置文档**:
...
| N2 | 方向一 M3（cron 记忆回填） | 高 | 用户最初反馈里"daemon 跑的任务没有 memory"更核心的一半，健康度趋势图（N1）能直接验收上线效果 | 中偏大 | 建议排在 N1 之后 |
