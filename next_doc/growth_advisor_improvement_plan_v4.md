...
- **版本**: v1（草案，方向级 + 关键设计点级规划）——**N1、N2、N3、N4
  已实施**，详见 `next_doc/growth_advisor_implementation_record.md`
  "N1：诊断面板健康度趋势化" / "N2：cron 记忆回填 M3" /
  "N3：关键词表 → tech_radar 种子同步" /
  "N4：外部资讯作为展示/报告背景" 四节；N5 尚未实施（v3 判断"先不做"，
  依赖 N2 观察效果后再评估，本文档不改变这个判断）。
- **前置文档**:
...
| N3 | 方向二 2.2 节（关键词 → tech_radar 种子同步） | 中 | 改动集中、默认关闭零风险，但收益依赖用户本身已经在用 tech_radar 功能 | 中 | 无强依赖，可与 N2 并行 | **已实施** |
| N4 | 方向二 2.3/2.4 节（外部资讯作为展示/报告背景） | 低 | 明确"仅展示、不影响判断"的克制设计，收益是锦上添花性质，不紧急 | 中 | 建议排在 N3 之后（依赖 2.2 节跑出的种子同步先验证数据链路通畅） | **已实施** |
