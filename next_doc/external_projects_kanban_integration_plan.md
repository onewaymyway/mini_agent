...
- 2026-08-28（同日追加）：**看板体验改进——异步型按钮的"处理中/已完成"
  提示**。上面几条修复解决了"trigger_run 会不会卡死 daemon"，但没有
  解决"用户点了「▶️ 触发」之后，页面在等响应期间看起来像卡住了，不
  知道是在正常处理还是坏了"这个体验问题——尤其是现在客户端超时放宽到
  960s 之后，长 entrypoint 场景下这段"看起来卡住"的时间可能有好几
  分钟。

  Streamlit 的执行模型是"一次脚本运行内同步阻塞"，没有真正的前端异步
  状态机，`st.spinner()` 是这个模型下的标准解法：包住那次耗时的
  HTTP 调用，调用期间原地显示旋转图标 + 提示文案，调用返回后自动
  消失、紧接着的 `st.success`/`st.error` 就是"已完成"的提示——不需要
  额外引入任务 ID/轮询/后台状态存储。改动落在
  `apps/mini_agent_kanban/app.py::render_external_projects_tab()` 的
  「▶️ 触发」按钮：提示文案里还带上了该 entrypoint 声明的
  `timeout_sec`（"该 entrypoint 最长可能运行 900 秒"），让用户对"要
  等多久"有个预期，而不是完全不知道。

  排查了同一个 tab 内其它按钮（注册项目/生成 review 模板/提交改进
  待办）：对应的后端实现（`registry.py`/`review.py`/`backlog.py`）
  都是纯文件读写 + YAML 解析，没有 subprocess 调用，耗时可忽略，
  不需要同样加 spinner——只有「▶️ 触发」这一个按钮背后是真正的
  外部子进程执行，属于文档标题里"异步型按钮"的唯一实例。
