...
## 5. 技术/组织/制度协同演化（对应参考文档第五十四节）【已完成】
...
**实施记录（2026-09-22）**：已按上述设计完成实施，默认窗口取
3 步（方案原文建议范围的上限，闭区间 `[s - window + 1, s]`）。
`app.py` 新增 `_collect_capability_kind_coevolution()`（纯统计
聚合函数，不做因果判断）/`_render_capability_kind_coevolution_
section()`（只读折叠区，紧跟"能力成熟度时间线"之后、"⭐ 首次达成的
里程碑"之前），明确标注"这只是时间上接近，不代表存在因果关系"。
未声明 `capability_kind` 的记录按 `"technology"` 兜底参与统计（同
第 4 节既有取舍），`step` 缺失的状态直接跳过。新增 10 个测试
（`tests/test_capability_kind_coevolution.py`，覆盖窗口边界闭区间、
自定义窗口宽度、跳过非法条目等），加上原有的全部通过
（569 passed）。详见 `PROJECT.md` 对应条目。
...
**实施记录（2026-09-22）**：已按上述设计完成实施。`app.py` 新增
`_CONFIDENCE_LABELS` 映射表 + `_confidence_label()`；
`page_knowledge()` 知识库浏览页条目标题改用中文标签展示，
排序逻辑（`_confidence_rank`）不受影响，继续按原始枚举值排序。
新增 7 个测试（`tests/test_confidence_label.py`），加上原有的全部
通过（576 passed）。详见 `PROJECT.md` 对应条目。

**至此，第十二轮方案第 1～6 节全部完成。**
