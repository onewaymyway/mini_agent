...
## 六、追加：修复"候选列表里出现标题是「空内容」的垃圾候选"

用户反馈目标树里偶尔会出现看起来"空"的待确认候选。排查后发现不是标题
真的是空字符串（`_parse_candidates()` 对空标题的过滤一直是有效的），
而是 `GoalTreeDecomposer.build_prompt()` 第4条指令原文里包含"输出空
内容即可"这句话——部分模型在"确实拆不出来"时没有理解成"什么都不要
输出"，而是把这句指令的字面意思复述回来当成一行候选（用户提供的
`goal_tree_decompose_rejected.json` 片段里能看到 `"goal_xxx:输出空内容
即可"`、`"goal_xxx:空内容"` 这类被用户手动"忽略"过的记录，坐实了这个
根因）。这类文本对 `_parse_candidates()` 原有的"标题是否为空字符串"
判断来说是非空的，因此被当成正常候选放行、落盘展示。

修复两处，互为兜底：

- `build_prompt()` 第4条指令改掉容易被复述的措辞："如果完全拆不出来，
  就什么都不要输出——不要输出'空内容''无候选''拆不出来'这类说明性
  文字，那样的占位文字会被当成真正的候选标题，是不允许的。"降低模型
  产出这类占位文本的概率，是治标的第一道防线。
- `_parse_candidates()` 里新增 `_is_junk_title()` 过滤（治本的第二道
  防线，不依赖模型是否真的听话）：
  - `_JUNK_TITLE_EXACT`：一批太短/太通用、只能用"整行严格相等"判断的
    词（"无"/"暂无"/"none"/"n/a" 等）——这些词如果用"包含"判断会误伤
    "无线网络配置"这类正常标题；
  - `_JUNK_TITLE_SUBSTRINGS`：一批足够独特、可以用"包含"判断的占位
    短语（"空内容"/"无候选"/"无需拆分"/"拆不出"/"无法拆解"/"无子
    节点"等）——即使模型把这些短语混进一整句话里复述（比如"输出空
    内容即可"、"这个节点无法拆解，建议保持现状"），也能被拦下来；
  - 两者都基于 `_dedupe_title_key()` 归一化（小写、去标点、去空格）后
    再匹配，避免因为大小写/标点/空格差异漏判。
  过滤发生在"标题为空"检查之后、去重检查之前，命中即 `continue` 跳过
  该行，不计入候选数量上限。
- 新增测试 `test_parse_candidates_filters_junk_placeholder_titles`（覆盖
  "空内容"/"输出空内容即可"/"无候选"/"拆不出来"混在候选列表里能被过滤、
  真正有效的候选不受影响）与
  `test_parse_candidates_does_not_over_filter_legit_titles`（覆盖"无线
  网络配置优化"这类含"无"字的正常标题不会被误伤），跟
  `tests/test_goal_tree_phase2.py` 既有 37 个用例一起跑，共 39 个用例
  全部通过，无回归。
- 本次修复只影响候选**生成/解析**阶段，不处理历史数据——如果现有
  `goal_backlog.json` 里已经有历史遗留的"空内容"类垃圾候选，需要用户
  在看板上手动"忽略"清掉，或后续按需再补一个一次性清理脚本。
