...
- **CronScheduler**（`evolution/cron_scheduler.py`）：interval/cron 双格式，8 个内置系统 job（consolidation/wiki_gap_scan/wiki_fallback_cleanup/workdir_sync/self_eval/goal_review/digest_trim/self_maintain，后两个 wiki job 为下一阶段新增）；触发的 job 通过 `InputQueue.enqueue(initiator="cron")` 提交
