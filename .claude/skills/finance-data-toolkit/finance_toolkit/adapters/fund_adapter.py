...
基于 FundScraper 实现 Adapter 接口，支持：
- 基金净值 (fund_nav)
- 基金持仓 (fund_holdings)
- 基金排行榜 (fund_rank)
- 基金信息 (fund_info)
...
class FundAdapter(Adapter):
    """
    基金数据适配器
    
    封装 FundScraper，提供统一的 Adapter 接口。
    """
    
    def __init__(self):
        self._scraper = FundScraper()
    ...
    def description(self) -> str:
        return '公募基金数据适配器（东方财富）'
    
    async def initialize(self):
        """初始化适配器"""
        self._initialized = True
        logger.info(f"基金适配器初始化完成: {self.source_name}")
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"基金适配器健康检查失败: {e}")
            return False
    ...
    async def close(self):
        ...
        logger.info(f"基金适配器已关闭: {self.source_name}")
    ...
    def __repr__(self) -> str:
        return f"FundAdapter(source={self.source_name})"


# 便捷函数
async def create_fund_adapter() -> FundAdapter:
    """创建基金适配器实例"""
    adapter = FundAdapter()
