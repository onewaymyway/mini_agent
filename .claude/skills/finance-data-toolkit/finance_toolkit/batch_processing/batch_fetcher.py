...
def fetch_single_kline(symbol: str, port: int, tab_id: str, scale: str = '240', datalen: int = 1023) -> Dict[str, Any]:
    """抓取单只股票K线数据 (新浪财经)"""
    # 转换为新浪格式: 600000 -> sh600000
    sina_symbol = 'sh' + symbol if symbol.startswith('6') else 'sz' + symbol
    
    # 映射 scale 到新浪 scale 参数
    scale_map = {'1m': '5', '5m': '5', '15m': '15', '30m': '30', '60m': '60', 'daily': '240', 'weekly': '1200', 'monthly': '7200'}
    sina_scale = scale_map.get(scale, '240')
    
    url = f"https://quotes.sina.cn/cn/api/jsonp_v2.php/var=/CN_MarketDataService.getKLineData?symbol={sina_symbol}&scale={sina_scale}&ma=no&datalen={datalen}"
    ...
    for i, symbol in enumerate(symbols):
        ...
        try:
            ...
            if result.get('success'):
                ...
                # 保存为 backtest_framework 期望的格式
                # 格式: {symbol}.SH/.SZ_kline_raw_{timestamp}.json
                # 内容: {"kline_raw": [...], "symbol": "...", "timestamp": "..."}
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                # 转换为标准格式: 600000 -> 600000.SH, 000001 -> 000001.SZ
                std_symbol = f'{symbol}.SH' if symbol.startswith('6') else f'{symbol}.SZ'
                output_file = output_path / f'{std_symbol}_kline_raw_{timestamp}.json'
                save_data = {
                    'kline_raw': result.get('kline', []),
                    'symbol': std_symbol,
                    'timestamp': datetime.now().isoformat(),
                    'scale': scale,
                    'datalen': datalen
