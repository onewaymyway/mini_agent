...
class TestDongqiudiSearcher:
    """DongqiudiSearcher 单元测试"""
    
    def setup_method(self):
        self.searcher = DongqiudiSearcher()
    ...
    def test_source_name(self):
        """测试：来源名称"""
        assert self.searcher.source_name == "dongqiudi"
    
    def test_supported_types(self):
        """测试：支持的搜索类型"""
        assert self.searcher.supported_types == ["news_search", "match_data", "player_info", "team_info"]
    ...
    def test_search_players_exists(self):
        """测试：_search_players 方法存在"""
        assert hasattr(self.searcher, '_search_players')
        assert callable(self.searcher._search_players)
    ...
    def test_search_result_format(self):
        ...
        mock_result = {
            'source': 'dongqiudi',
            'title': '梅西转会新闻',
            'url': 'https://www.dongqiudi.com/news/123',
            'snippet': '梅西加盟迈阿密国际',
    ...
    def test_deduplication(self):
        """测试：去重功能"""
        results = [
            {'source': 'dongqiudi', 'title': '重复', 'url': 'https://dongqiudi.com/same', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
            {'source': 'dongqiudi', 'title': '重复', 'url': 'https://dongqiudi.com/same', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
            {'source': 'dongqiudi', 'title': '唯一', 'url': 'https://dongqiudi.com/unique', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
        ]
        seen = set()
        unique = []
        for r in results:
    ...
    def test_max_results_limit(self):
        """测试：最大结果数限制"""
        results = [
            {'source': 'dongqiudi', 'title': f'结果{i}', 'url': f'https://dongqiudi.com/{i}', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'}
            for i in range(30)
        ]
    ...
    def test_invalid_url_filtering(self):
        """测试：无效 URL 过滤"""
        results = [
            {'source': 'dongqiudi', 'title': '有效', 'url': 'https://dongqiudi.com/valid', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
            {'source': 'dongqiudi', 'title': '无效', 'url': '', 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
            {'source': 'dongqiudi', 'title': '无效', 'url': None, 'metadata': {}, 'scraped_at': '2026-08-06T08:00:00'},
        ]
        valid = [r for r in results if r.get('url')]
        assert len(valid) == 1
        assert valid[0]['url'] == 'https://dongqiudi.com/valid'
    ...
    def test_base_url(self):
        """测试：基础 URL"""
        from src.searchers.dongqiudi_search import DQD_BASE
        assert DQD_BASE == "https://www.dongqiudi.com"


class TestDongqiudiSearcherIntegration:
    """DongqiudiSearcher 集成测试"""
