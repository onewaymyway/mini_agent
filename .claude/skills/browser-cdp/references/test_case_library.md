...
- **场景化测试**: 电商、新闻、搜索、社交、表单等常见网站类型
- **专项测试**: 动态内容、SPA、AJAX、无限滚动等复杂交互场景
- **基类模板**: 统一的测试框架和断言方法
...
| **新增单元模块** | **文件** | **描述** |
| browser_console | test_browser_console.py | JS执行、console日志、网络请求抓取、cookie管理 |
| browser_watch | test_browser_watch.py | URL/标题等待、轮询判断操作完成 |
| cdp_client | test_cdp_client.py | CDP客户端连接、tab管理、CDPSession命令发送 |
| retry_policy | test_retry_policy.py | 重试配置、可重试操作、with_retry装饰器 |
| circuit_breaker | test_circuit_breaker.py | 熔断状态机、失败追踪器、故障率计算 |
| utils | test_utils.py | 连接辅助函数、JSON打印、URL解析 |

---

## 3. 详细测试用例说明

### 3.1 电商购物流程测试 (test_ecommerce_flow.py)
...
### 3.2 新闻资讯提取测试 (test_news_extraction.py)
...
### 3.3 搜索引擎测试 (test_search_engine.py)
...
### 3.4 社交媒体测试 (test_social_media.py)
...
### 3.5 表单提交测试 (test_form_submission.py)
...
### 3.6 动态内容专项测试 (test_dynamic_content.py)
...
# 按优先级筛选测试
python -m pytest browser-cdp/templates/ -k "P0"
...
|----------|--------|----------|----------|----------|------|
| ECOM-04 | P1 | mock 设置不正确 | AttributeError: 'NoneType' object has no attribute 'click_selector' | 检查 mock 对象返回值 | 待修复 |
