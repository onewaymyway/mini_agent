...
class ExceptionHandler:
    ...
    def handle(
        ...
        **kwargs,
        ...
        try:
            return get_retry_config(operation).retry_operation(
                wrapped_func,
                config=config,
                operation=operation,
            )
...
def with_exception_handling(operation: str = "unknown"):
    """
    装饰器：为函数添加统一异常处理
    
    Usage:
        @with_exception_handling("search")
...
def async_with_exception_handling(operation: str = "unknown"):
    """
    异步装饰器：为函数添加统一异常处理
    
    Usage:
        @async_with_exception_handling("search")
