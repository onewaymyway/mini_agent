...
class ZhihuSearch(BaseSearcher):
    ...
    def __init__(self, **kwargs):
        ...
        self.search_url_template = "https://www.zhihu.com/search?type=content&q={query}"
    ...
    def supported_types(self) -> list:
        return ["zhihu_question", "zhihu_answer", "zhihu_article", "zhihu_topic"]
    ...
    def get_detail(self, url: str, config: Optional[SearcherConfig] = None) -> Dict:
        """获取详情页内容"""
        cfg = config or self.config
        detail = {'url': url, 'source': self.source_name, 'type': 'zhihu'}
...
def search_zhihu_via_baidu(query: str, max_results: int = 20, **kwargs) -> SearchResults:
    """通过百度搜索知乎内容的便捷函数"""
    return ZhihuSearch(**kwargs).search(query, max_results=max_results, **kwargs)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='知乎搜索器')
    parser.add_argument('query', help='搜索关键词')
    parser.add_argument('--max-results', type=int, default=20)
    parser.add_argument('--output-dir', default='./search_results')
    parser.add_argument('--port', type=int, default=9333)
    parser.add_argument('--name', default='zhihu_search')
    parser.add_argument('--headless', action='store_true')
    args = parser.parse_args()
    
    searcher = ZhihuSearch(port=args.port, headless=args.headless)
