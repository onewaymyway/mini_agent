...
class YahooSearcher(BaseSearcher):
    def __init__(self, **kwargs):
        ...
        self.search_url = "https://search.yahoo.com/search?p={query}"
    ...
    def search(self, query: str, max_results: int = 20, **kwargs) -> SearchResults:
        ...
        for i in range(min(max_results, 10)):
            result = SearchResult(
                title=f"{query} - Yahoo 第{i+1}条",
                url=f"{self.base_url}/search?p={query}",
                snippet=f"Yahoo 搜索: {query}",
