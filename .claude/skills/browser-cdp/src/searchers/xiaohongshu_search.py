...
class XiaohongshuSearcher(BaseSearcher):
    ...
    def supported_types(self) -> List[str]:
        return ["note", "user", "product", "topic"]
    ...
    async def _extract_results(self, browser, search_type: str, query: str) -> List[SearchResult]:
        ...
        (() => {{
            ...
            // 小红书笔记搜索结果
            document.querySelectorAll('.note-item, .search-note-item, [class*="note"]').forEach(el => {{
                const titleEl = el.querySelector('.title, .note-title, h3, .note-card-title');
                const linkEl = el.querySelector('a[href*="/explore/"], a[href*="/discovery/item/"]');
                const authorEl = el.querySelector('.author, .user-name, .note-author');
                const likesEl = el.querySelector('.like-count, .likes, .count');
                const coverEl = el.querySelector('img[data-src], img[src*="xhscdn"]');
                
                if (titleEl && linkEl) {{
                    const authorText = authorEl ? '作者: ' + authorEl.innerText.trim() : '';
                    results.push({{
                        source: 'xiaohongshu',
            ...
            if (results.length === 0) {{
                document.querySelectorAll('.result-item, .list-item, [class*="result"]').forEach(el => {{
                    ...
                    if (titleEl && linkEl && linkEl.href.includes('xiaohongshu.com')) {{
                        results.push({{
                            source: 'xiaohongshu',
                            title: titleEl.innerText.trim(),
                            url: linkEl.href,
    ...
    async def get_detail(self, url: str, config: Optional[SearcherConfig] = None) -> Dict:
        ...
        try:
            ...
            (() => {
                const result = {
                    title: document.querySelector('.title, h1, .note-title')?.innerText || '',
                    content: document.querySelector('.content, .note-content, .markdown-body')?.innerText || '',
                    author: document.querySelector('.author, .user-name')?.innerText || '',
                    likes: document.querySelector('.like-count, .likes')?.innerText || '',
                    comments: document.querySelector('.comment-count')?.innerText || '',
                    tags: Array.from(document.querySelectorAll('.tag, .hashtag')).map(el => el.innerText.trim()),
...
def main():
    ...
    parser.add_argument("--type", choices=["note", "user", "product", "topic"],
