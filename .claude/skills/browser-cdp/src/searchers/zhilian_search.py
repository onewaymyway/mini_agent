...
class ZhilianSearcher(BaseSearcher):
    ...
    async def search(self, query: str, config: SearcherConfig,
        ...
        try:
            ...
            # 构建搜索URL
            search_url = f"https://sou.zhaopin.com/?jl={city or '北京'}&kw={query}"
            ...
            (() => {
                const results = [];
                document.querySelectorAll('.soupager__item, .job-item').forEach(el => {
    ...
    async def get_detail(self, url: str, config: SearcherConfig) -> Dict:
        ...
        try:
            ...
            (() => {
                ...
                const description = document.querySelector('.job-detail')?.innerText || '';
...
def main():
    parser = argparse.ArgumentParser(description="智联招聘搜索")
    ...
    parser.add_argument("--output-dir", default="./search_results/zhilian", help="输出目录")
    ...
    async def run():
        ...
        if results:
            save_results(results, args.output_dir, "zhilian")
