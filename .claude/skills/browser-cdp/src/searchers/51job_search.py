...
@dataclass
class FiveOneJobConfig(SearcherConfig):
    """前程无忧搜索器专用配置"""
    # 搜索参数
    city: str = ""  # 城市名称
...
class FiveOneJobSearcher(BaseSearcher):
    """
    前程无忧搜索器
    
    使用方式：
        config = FiveOneJobConfig(query="Python开发", city="北京", max_results=20)
        searcher = FiveOneJobSearcher(config=config)
        results = searcher.search()
        results.save_json('output/51job_results.json')
    """
    ...
    def __init__(self, config: FiveOneJobConfig = None):
        super().__init__(config or FiveOneJobConfig())
        self._session = None
    ...
    async def search(self, query: str, config: Optional[SearcherConfig] = None) -> List[SearchResult]:
        ...
        results = SearchResults(source="51job", query=query)
    ...
    def _perform_search(self, query: str):
        """执行搜索操作"""
        # 在搜索框中输入关键词
        search_box = self.session.query_selector('#keyword')
        if search_box:
            search_box.click()
            ...
            # 点击搜索按钮
            search_btn = self.session.query_selector('#search_btn')
            if search_btn:
                search_btn.click()
                time.sleep(2)
    ...
    def _extract_jobs_from_page(self) -> List[JobInfo]:
        ...
        (function() {
            const jobs = [];
            const cards = document.querySelectorAll('.el, .job_item');
            
            cards.forEach(card => {
                const job = {
                    title: card.querySelector('.job-name')?.textContent?.trim() || 
                           card.querySelector('[class*="job-name"]')?.textContent?.trim() || '',
                    ...
                    published_time: card.querySelector('.time')?.textContent?.trim() || '',
    ...
    def _parse_job_card(self, card_data: Dict) -> Optional[JobInfo]:
        ...
        try:
            ...
            job = JobInfo(
                ...
                published_time=card_data.get('published_time', '').strip(),
    ...
    def _extract_job_detail(self) -> Dict:
        ...
        (function() {
            return JSON.stringify({
                ...
                location: document.querySelector('.address')?.textContent?.trim() || '',
                experience: document.querySelector('.experience')?.textContent?.trim() || '',
                education: document.querySelector('.education')?.textContent?.trim() || '',
                description: document.querySelector('.job-detail')?.textContent?.trim() || '',
                benefits: Array.from(document.querySelectorAll('.tag span')).map(s => s.textContent.trim())
            });
        })()
        """
    ...
    async def search_batch(self, queries: List[str], **kwargs) -> SearchResults:
        ...
        Returns:
            合并后的搜索结果
        """
        all_results = SearchResults(source="51job", query="batch")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='前程无忧职位搜索器')
