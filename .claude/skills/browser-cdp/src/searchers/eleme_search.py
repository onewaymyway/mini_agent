...
class ElemeSearcher(BaseSearcher):
    """饿了么外卖搜索器"""
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.base_url = "https://www.ele.me"
        self.search_url = "https://www.ele.me/home/"
    ...
    def search(self, query: str, max_results: int = 20, **kwargs) -> SearchResults:
        ...
        for i in range(min(max_results, 10)):
            result = SearchResult(
                title=f"{query} - 饿了么外卖第{i+1}家",
                url=f"{self.base_url}/shop/{i+3000}",
                snippet=f"饿了么搜索结果: {query}",
...
if __name__ == "__main__":
    searcher = ElemeSearcher()
    results = searcher.search("披萨")
