#!/usr/bin/env python
"""
zhibo8_search.py - 直播吧体育搜索器

使用 browser-cdp skill 搜索直播吧，获取体育新闻、赛事数据、社区内容等。

用法:
    python zhibo8_search.py "NBA"
    python zhibo8_search.py "足球" --type news --output-dir ./zhibo_results
"""
...
# ========== 直播吧专用配置 ==========
ZHIBO_BASE = "https://www.zhibo8.cc"
ZHIBO_NEWS_URL = f"{ZHIBO_BASE}/zhibo8/"
ZHIBO_SEARCH_URL = f"{ZHIBO_BASE}/search/"
ZHIBO_NBA_URL = f"{ZHIBO_BASE}/nba/"
ZHIBO_FOOTBALL_URL = f"{ZHIBO_BASE}/zuqiu/"

# 默认输出目录
ZHIBO_OUTPUT_DIR = SKILL_DIR.parent / "search_results" / "zhibo8"


class Zhibo8Searcher(BaseSearcher):
    """直播吧体育搜索器"""
    ...
    def search(
        ...
        try:
            ...
            output_path = output_dir or str(ZHIBO_OUTPUT_DIR)
            save_results(results, output_path)
            print(f"[直播吧] 搜索完成，共获取 {len(results)} 条结果")
            return results
    ...
    def _search_news(self, browser, query: str, max_results: int, wait_timeout: int) -> List[Dict]:
        results = []
        try:
            url = f"{ZHIBO_SEARCH_URL}?keyword={quote(query)}"
            browser.get(url, timeout=wait_timeout)
            time.sleep(random.uniform(2, 4))
            news_items = browser.query_selector_all(".news-item, .post-item, .list-item, .content-list li")
                ...
                try:
                    ...
                    if title_elem and link_elem:
                        ...
                        if title and link:
                            results.append({
                                "title": title,
                                "url": link if link.startswith("http") else f"{ZHIBO_BASE}{link}",
            ...
            print(f"[直播吧新闻搜索] 错误: {e}")
        return results

    def _search_matches(self, browser, query: str, max_results: int, wait_timeout: int) -> List[Dict]:
        results = []
        try:
            url = f"{ZHIBO_SEARCH_URL}?keyword={quote(query)}&type=match"
            browser.get(url, timeout=wait_timeout)
            time.sleep(random.uniform(2, 4))
            match_items = browser.query_selector_all(".match-item, .game-item, .score-item")
...
def main():
    parser = argparse.ArgumentParser(description="直播吧体育搜索器")
    parser.add_argument("query", help="搜索关键词")
    parser.add_argument("--type", default="all", choices=["news", "match", "all"])
