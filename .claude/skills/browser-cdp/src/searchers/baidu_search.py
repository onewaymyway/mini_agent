...
class BaiduSearcher(BaseSearcher):
    ...
    def _build_search_url(self, query: str, page: int = 1) -> str:
        ...
        if page > 1:
            return f"{self.base_url}/s?wd={encoded_query}&pn={(page-1)*10}"
...
def search_baidu(query: str, max_results: int = 20, **kwargs) -> SearchResults:
    """百度搜索便捷函数"""
    return BaiduSearcher(**kwargs).search(query, max_results=max_results, **kwargs)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='百度搜索器')
    parser.add_argument('query', help='搜索关键词')
    parser.add_argument('--max-results', type=int, default=20)
    parser.add_argument('--output-dir', default='./search_results')
    parser.add_argument('--port', type=int, default=9333)
    parser.add_argument('--name', default='baidu_search')
    parser.add_argument('--headless', action='store_true')
    args = parser.parse_args()
    
    searcher = BaiduSearcher(port=args.port, headless=args.headless)
