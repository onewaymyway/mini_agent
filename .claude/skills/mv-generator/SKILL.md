...
- **拼音模糊匹配（需要 `pypinyin`）**：上面这条"整句级别模糊匹配"原本
  只比较汉字，中文 ASR 出错很大一部分是同音字/近音字替换（比如把
  "科技的窍门"识别成"可惜的窗门"），字形上完全不匹配但读音一样，纯
  汉字比较会直接判定"整句不相似"退化成插值。脚本额外把这一行和候选
  ASR 片段都转成拼音序列再比一次相似度，汉字/拼音两档**分开独立打分、
  分开设置确认阈值**（`--hanzi-confirm-threshold`/
  `--pinyin-confirm-threshold`，拼音档默认门槛明显更高，因为纯拼音串
  比较天然噪声更大），同音字错误也能命中，明显减少退化成插值的行数。
  未装 `pypinyin` 时自动跳过这一条（拼音档恒为 0 分，不会被采信），
  不会报错，可以放心直接使用，不需要先判断装没装拼音库。
...
- **窗口匹配加入字数对齐 + 更明确的匹配优先级**：窗口模糊匹配阶段，
  ...
  复核。
- **修复了实测暴露的"一行没匹配上、拖垮全曲"的级联故障**：一行没有
  真实对应 ASR 文本的歌词（纯语气词/和声/间奏，或者这段真实演唱音频
  ASR 识别彻底失败），过去的版本会**逐行、立即、不可撤销地**决策——
  就近窗口内找不到够格的匹配就放宽窗口再找一次，只要分数勉强过一个
  不太高的门槛就立刻采信、游标立刻跳过去；一旦这个"矮子里拔将军"的
  弱匹配恰好抓到了本该属于后面某一行的 ASR segment，游标和 ASR 搜索
  指针会被带偏几十上百秒，连带把中间跳过的这一大段真实 ASR 内容全部
  甩掉，后面十几行会一路级联退化成插值，彻底脱离真实演唱进度。

  现在改成**"挂起队列 + 区间插值"**：拿不到高置信度锚点/够不到确认
  阈值的行，不再立即采信弱匹配，而是先放进挂起队列，**不移动游标、
  不移动搜索指针**，交给后面找到的下一个高置信度确定点，回头把这一
  整段挂起的行按**文本字数比例**，均分插值到"上一个确定点结束"到
  "这个确定点开始"之间这段真实存在的时间区间里；曲终仍未被回收的
  挂起行（比如结尾渐弱的语气词），用最后一个确定点到音频总时长之间
  的区间统一插值。这样"一行没对应文本"造成的影响最坏也只局限在这
  一行本身（及紧邻的、同样没对应文本的相邻行），不会传染给后面任何
  本来能对上的行。

  配套把拼音档的"确认阈值"从旧版隐含约 0.26（`0.28` 门槛 × `0.92`
  折扣）大幅收紧到默认 `0.6`（放宽窗口后 `0.72`）：用一首真实歌曲的
  完整 ASR 结果（《勇者与恶龙》，34 段 ASR segment、51 行歌词，其中
  一整段 8 秒多的真实演唱因 ASR 输出乱码占位字符而完全没有对应文本）
  实测验证过，旧版从第 8 行起就被一个 0.39 分的弱拼音匹配带偏，后面
  几十行全部级联跑偏、部分时间戳甚至跑到了音频总时长之外；修复后
  全曲 51 行全部单调递增、紧跟真实演唱进度，只有 8 行（2 行确实没有
  对应 ASR 文本的正文 + 6 行结尾语气词"la la la"/"oh oh oh"）被正确
  标记为区间插值，落在真实的音频空隙里，交给人工复核即可。
