...
description: 帮助用户创建符合 mini_agent 新版"文件夹模式"规范的 workflow（.agent/workflows/<name>/workflow.yaml，可携带私有 agents/skills/prompts）。当用户说"帮我创建一个workflow"、"做一个工作流"、"写一个xxx流水线"、"把这个workflow转成文件夹模式"时使用。
...
| `name` | 建议 | 可读名称 |
| `type` | 建议显式写 | 见下方"Step 类型"表；不写时按 `role` 是否为空自动推断为 `agent`/`role_agent`（向后兼容旧写法，新建议一律显式写 `type`） |
...
| `allow_parallel` | 否 | 默认 `true`；step 有隐式副作用（读写同一外部状态）时显式设为 `false` 强制串行 |
| `max_turns` | 否 | 该 step 允许的最大 LLM 轮数，默认 10 |
| `model` | 否 | 覆盖该 step 用的模型，缺省继承全局配置 |
| `timeout` | 否 | 该 step 超时（秒） |
| `retry_on_gate_fail` | 否 | evaluator 质检门不达标时重跑前序步骤的最大次数，仅对 `role_type: evaluator` 的角色有意义 |
| `retry_on_error` | 否 | 普通异常（超时/工具报错）重试次数 |
...
3. **创建目录结构**：`.agent/workflows/<name>/`，按需创建 `agents/`、
   `skills/`、`prompts/`。
4. **写 `prompts/*.md`**（如果用文件夹模式且 prompt 较长）：每个文件只放
   一个 step 的 prompt 正文，占位符规则同上。简短的 prompt（一两句话）
   可以直接内嵌 `prompt` 字段，不强制拆文件。
5. **写 `agents/<role>.md`**（如果某个 step 需要专属角色）：格式参考
...
6. **写 `skills/<skill-name>/SKILL.md`**（如果某个 step 需要强制挂载某项
...
7. **写 `workflow.yaml`**：按上方字段表填写，`steps` 里体现第 1 步确认的
   拓扑结构。
8. **自检**（写完后必须做，不要跳过）：
   - 每个 `id` 唯一、`depends_on` 引用的 id 都存在
   - `prompt`/`prompt_file` 至少有一个非空（`human_input` 除外）
