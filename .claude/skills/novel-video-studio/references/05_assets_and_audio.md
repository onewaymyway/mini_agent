...
**依赖 skill**：`gen_image_with_text`（按需补生成新实体/变体定妆图时用）。
...
**本阶段不再负责角色/地点定妆图的首次批量生成**——那是阶段3
（`references/02_global_assets.md`）的职责，在阶段2抽取出全局角色/
地点主表之后就已经集中生成并校验完成。进入本阶段循环时，正常情况下
当前大场景引用到的角色/地点应该都已经有 `asset_path`。本阶段的
Step 1 只处理两类"事后补丁"场景：

1. 阶段5（详细规划）按 SKILL.md §2.2 回补出的**新角色/地点**（阶段2
   没抽到、详细规划时才发现引用了原文里没被单独抽取的实体）；
2. 阶段5标注了 `character_variant_overrides`/
   `location_variant_overrides` 引用到的**外观变体**——变体图片本身
   就设计为"用到哪个场景才生成"，不属于阶段3的批量生成范围（见
   `02_global_assets.md` 末尾说明）。

阶段7 `generate_scene_videos_v2.py` 仍然会在生成任何 clip 之前硬性
检查角色/地点是否已有定妆图，缺失直接拒绝启动（见
`references/06_scene_video_generation.md`）——正常流程里这一关不应该
被触发（阶段3已经保证主表齐全），只有上面两类补丁场景遗漏时才会在
这里被拦下，发现报错回本阶段 Step 1 补生成即可。

**外部依赖**：CosyVoice（可选，配 `novel_tts_env`）、edge-tts（兜底）、
ffmpeg/ffprobe、`AGNES_API_KEY`（补生成定妆图时用）。API 失败处理见
`error_handling.md`。
...
├── global/
│   ├── characters.json / locations.json   # 本阶段按需回填新增实体/变体的 asset_path
...
SKILL.md §2.3 的循环，处理完某个大场景的阶段5规划后，配音只给这一个
大场景配（`--macro-id macro_XX`），配完直接进入阶段7生成这个大场景的
视频，而不是先把所有大场景的配音都配完再统一进入阶段7——用户要能在
看到第一个大场景成片的同时，其它大场景还没开始配音，这是正常状态，
不是"漏跑了"。

Step 1 的按需补生成同理：只处理当前大场景回补出的新实体/引用到的
变体，不需要、也不应该攒到所有大场景一起生成。

## Step 1：新增角色/地点/外观变体定妆图（按需补）

扫描 `global/characters.json`/`locations.json`，找出 `asset_path`
仍为 `null` 的**顶层条目**——正常流程里应该只有阶段5回补出的新实体
才会出现这种情况（阶段3已经把当时已知的主表全部生成完），对每一项
调用：
...
封面图（阶段3已经处理过，这里通常不需要重复）逻辑同角色定妆图，落到
`global/assets/cover.png`，供阶段8可选叠加。

**外观变体（`appearance_variants`）定妆图**：扫描每个角色/地点条目的
`appearance_variants[]`，找出 `asset_path` 仍为空、且**已经被阶段5
某个大场景的 `character_variant_overrides`/`location_variant_overrides`
实际引用到**的变体（不需要提前把全部理论上可能存在的变体都生成一遍，
按需生成即可），用该变体的 `visual_override_en` 生成一张定妆图：
...
作为硬性 error 拦下，进不了阶段7，所以正常流程里不应该依赖这个退化行为，
发现报错就应该回这里补生成变体定妆图。
...
（`check_assets_and_audio_v2.py` 会报错），**不允许忽略**——回阶段5
...
`macro_scene_*/scene_detail.yaml`——还没跑到阶段5的大场景根本不会有
这个文件，所以校验结果天然只覆盖"已经规划到当前进度"的场景，不会因为
后面的大场景还没配音就报错。**退出码非 0 不允许交付阶段7**，按报错
...
进入阶段7，为每个 `micro_scene` 生成视频 clip，并在每个大场景内合成
