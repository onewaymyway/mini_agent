...
阶段5产出的 `scene_detail.yaml` 里**没有** `prompt_en` 字段（阶段5只
...
- **不允许**因为"这个角色/地点还没生成定妆图，先随手把 video_mode 改成
  text 让脚本能跑"——这是在悄悄放弃一致性，正确做法永远是先回阶段6
...
`check_assets_and_audio_v2.py`（阶段6 Step3）仍然建议先跑一遍——它能给
...
（阶段2锁定的核心外观特征，见 `01_entity_extraction.md`）。写
...
`04_scene_detail_planning.md` Step 4.5），改成摘抄该变体的
...
`compose_final_video_v2.py`（见 Step 4、`07_final_compose.md`）都建议
传 `-1`，其余不涉及批量视频生成/本地渲染的步骤沿用默认超时即可。

- **按 SKILL.md §2.3 的逐场景循环，默认必须带 `--macro-id` 只处理当前
  刚配完音的这一个大场景**——不要跑完一个大场景的阶段6就去跑下一个
  大场景的阶段6，而是紧接着用这一步把当前大场景的视频也生成、合成
...
SKILL.md「关于视频节奏对齐」一节）。这个前提是阶段6回填的
`duration_sec` 本身要落在合理范围内（对应 `05_assets_and_audio.md`
的时长超限处理规则，以及 `04_scene_detail_planning.md` 新增的粗估
...
才回到阶段5开始下一个大场景的规划——不要在用户还没看过这个大场景之前
就接着跑下一个大场景的阶段5/6。全部大场景都变成 `status: done` 后，
再进入阶段8。可以随时用 `check_project_state.py` 查看还有哪些大场景
没到 `done`。
...
2. **`video_mode: reference` 但提示"没有可用的参考图片"**：说明对应
   ...
   生成（不会整体失败），但画面一致性会打折扣，建议回阶段6补生成缺失
   定妆图，再用 `--force` 重新生成这个场景；
3. **`compose_macro_scene.py` 报错"缺少配音"**：说明阶段6的配音脚本
