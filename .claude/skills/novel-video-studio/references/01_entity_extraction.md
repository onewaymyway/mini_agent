...
### Step 2.55：地点粒度——同一建筑/地名如果覆盖多个视觉差异很大的子
### 空间，必须拆成独立地点条目，不能只登记一条笼统的锚点

`visual_anchor_en` 只有一句话的容量，只能锁定"一个视觉上相对统一的
空间"该有的样子。如果原文里同一个地名/建筑，会在不同场景里展示视觉
差异很大的多个子空间——最典型的是**室外全貌 vs 室内房间**，其次是
**同一室内的不同房间**（客厅/卧室/地下室）——把它们强行登记成同一个
地点条目、共用一条 `visual_anchor_en`，会导致这条锚点天然覆盖不到其中
一部分子空间：Agent 写这部分子空间的 `prompt_en` 时无锚点可摘抄，只能
每次临场编一遍环境细节，这正是"同一场景反复出现却每次画面都不一样"
的常见根因之一，和角色外观漂移是同一类问题、同一个解法。

**判断标准**（满足即应拆分）：这个地点在原文里会被展示为两个及以上
"如果分别拍出定妆图，一眼就能看出不是同一个空间"的场景（例如"雪夜
中的房屋外观"和"屋内摆着沙发的客厅"）。反例：同一个房间在白天/夜晚
光线不同，或者镜头远近不同，这类差异用 `appearance_variants`（见下面
Step 2.6）处理即可，不需要拆成独立地点条目——**地点拆分解决"根本不是
同一空间"的问题，`appearance_variants` 解决"同一空间的外观随剧情变化"
的问题，两者不要混用**。

**拆分做法**：保留原地点 id 作为其中一个子空间（通常是使用频率最高或
原文着墨最多的一个，比如外观镜头较多就保留给外观），为其余子空间新建
独立地点条目，命名 `<原id>_<子空间简称>`（如 `loc_01_ext` 外观、
`loc_01_living` 客厅、`loc_01_hallway` 玄关），各自登记独立的
`name`/`description_zh`/`description_en`/`visual_anchor_en`，`relations`
/`era_setting` 等可以直接复用原条目的取值。已经在用原地点 id 的
