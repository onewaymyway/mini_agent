...
description: 把一篇小说自动转换成一支配旁白/角色对话配音的成片视频。一体化流程：小说→剧本转换+剧本校验（自然语言剧本，明确区分旁白/对话与说话人，先核实忠实于原文再往下走）→全局角色地点抽取（含声音设定与锁定视觉锚点）→全局角色/地点定妆图集中生成与校验→大场景切分（弹性时长，默认≤120秒，剧情连续性强时可放宽到≤240秒）→单大场景详细规划（旁白/对话按剧本行直接映射）→素材补全与差异化配音生成→角色/场景一致性校验+小场景视频生成与大场景内合成→最终拼接转场。每个阶段的具体操作步骤放在 references/ 下，进入某阶段前按需加载对应文件，完成后可从上下文中卸载。支持断点续跑、失败重试与 API Key 自动切换、跨 session 续接、以及用户对已生成内容提反馈后的定向回退重跑。当用户说"把这本小说做成视频"、"小说转视频"、"生成小说解说视频"时使用本 skill。
triggers: 小说转视频, 小说做视频, 小说解说视频, novel to video, 角色配音视频, 小说视频生成
resources:
  - id: script-conversion
    path: references/00_script_conversion.md
    description: 阶段0——小说→剧本转换（自然语言剧本，`旁白：...`/`角色名：台词` 结构化格式，从源头分离旁白/对话/说话人）+ 阶段1剧本校验（Agent 逐场次核对剧情覆盖/对话覆盖/说话人归属/无臆造，核查结论写入 script_review.yaml，check_script.py 只做机械把关），是后续所有阶段的输入源
    triggers: 剧本转换, 剧本化, script.md, 剧本校验, script_review, 场次, 旁白, 对话行, 说话人归属, 剧情覆盖, 无臆造
  - id: entity-extraction
    path: references/01_entity_extraction.md
    description: 阶段2——全局角色/地点抽取（含 voice_profile 声音设定 + visual_anchor_en 锁定视觉锚点 + appearance_variants 外观变体登记，供阶段7一致性核查使用），也覆盖阶段5触发的单点补抽取模式；输入源是阶段0产出的 script.md，不是小说原文
  ...
  - id: scene-detail-planning
    path: references/04_scene_detail_planning.md
    description: 阶段5——单个大场景详细规划：拆小场景（micro_scene，需按视频生成接口4-12秒硬限规划时长+粗估时长自查）、旁白/对话按剧本行直接映射为 content_blocks（不再需要现场抠引号/识别动作描写混入）、外观变体检测与 character_variant_overrides/location_variant_overrides 标注、visual_hint 撰写要求、check_scene_detail.py 校验规则与自查清单
  ...
  - id: assets-and-audio
    path: references/05_assets_and_audio.md
    description: 阶段6——素材补全（仅阶段5回补出的新角色/地点、以及被引用到的外观变体，按需生成，不是全局批量生成）+ 按角色差异化配音（TTS）
    triggers: 定妆图补生成, 配音, tts, voice_profile, asset_path, 差异化配音, 外观变体定妆图
  - id: scene-video-generation
    path: references/06_scene_video_generation.md
    description: 阶段7——手写 prompt_en/video_mode（默认 reference，缺定妆图时生成前硬性拦截，不再静默降级为 text） + 角色/场景/情节一致性核查（完全由 Agent 逐条语义核查、对照角色档案+变体+visual_hint/content_blocks 情节原文，核查结论写入 consistency_report.yaml，脚本 check_consistency_report.py 只做机械把关：覆盖完整性+全部pass+未过期，不做语义判断）+ 小场景视频生成(generate_scene_videos_v2.py，timeout必须传-1)与大场景内合成(compose_macro_scene.py，逐场景慢放/快放对齐规划时长)
...
即可，文件不会变）。十份子资源分别对应九个阶段（阶段0——小说→剧本转换、
阶段1——剧本校验、阶段2——角色/地点抽取、阶段3——全局定妆图生成、
阶段4——大场景切分、阶段5——详细规划、阶段6——素材补全+配音、
阶段7——视频生成+合成、阶段8——最终拼接；其中阶段0/1合记在同一份
`script-conversion` 子资源里，其余每个阶段各自一份）+ 两条贯穿性规范
...
├── check_script.py               # 阶段1校验（剧本核查报告的机械把关）
├── check_entities.py             # 阶段2校验
├── check_macro_scenes.py         # 阶段4校验（时长弹性化：默认≤120秒建议值，≤240秒硬上限）
├── check_scene_detail.py         # 阶段5校验
├── voice_mapping.py / tts_engine.py / synthesize_scene_audio.py
│                                  # 阶段6配音（synthesize_scene_audio.py 是入口）
├── check_assets_and_audio_v2.py  # 阶段6校验
├── generate_scene_videos_v2.py   # 阶段7生成小场景视频
...
阶段3没有独立的 `check_*.py` 脚本（当前未实现，由 Agent 直接读取
`global/characters.json`/`locations.json` 逐条核对 `asset_path`，见
`references/02_global_assets.md`）。
...
阶段0 小说→剧本转换（script.md，全文一次性）
  → 阶段1 剧本校验（script_review.yaml，Agent逐场次核对剧情/对话/说话人/无臆造）
  → 阶段2 novel-entity-extractor（角色/地点抽取，含 voice_profile，基于 script.md 全文一次性）
  → 阶段3 全局角色/地点定妆图集中生成与校验（对阶段2产出的主表逐条生成，全部完成才能往下走）
  → 阶段4 大场景切分（macro_scene，以剧本场次为单位归并/细分，全文一次性，弹性时长）
  → 对每个大场景 macro_scene_XX 依次循环：
        阶段5 详细规划(该场景) → 阶段6 素材补全(按需，仅新增实体/变体)+配音(该场景)
        → 阶段7 小场景视频生成+大场景内合成(该场景) → 向用户展示、等确认
  → 全部大场景 done 后，阶段8 最终拼接转场
```

**关键变化1（剧本化）**：阶段2、阶段4、阶段5现在读的都是阶段0产出的
结构化剧本 `script.md`，不再直接碰小说原文——原文里"叙事视角混杂对话、
引号嵌套动作描写"的非结构化特性是过去大场景/小场景切分反复出问题的
根因，剧本化把"区分旁白/对话/说话人"这件事一次性提前做对，下游阶段5
不再需要现场抠引号、识别动作描写混入。同时大场景时长约束从硬性 ≤120秒
改为弹性建议值（剧情连续性强时允许放宽到 ≤240秒），避免为了凑时长机械
切断连续戏剧张力。详见 `next_doc/novel_video_studio_fix_plan_v5.md`。

**关键变化2（全局定妆图独立成阶段）**：角色/地点定妆图生成以前是
"素材+配音"阶段的第一步，按 `asset_path` 是否已生成去重、"用到哪个
大场景就顺带生成"。这个设计对阶段2主表之后才发现的新角色/新地点仍然
合理，但对阶段2一次性抽取出的、贯穿全书的角色/地点主表来说不够——
定妆图是保证角色/场景外观跨大场景一致的锚点资源，理应在角色/地点主表
定稿之后就集中生成、集中校验，而不是拖到"用到哪个大场景才发现没生成"。
现在阶段2产出完整主表后，先进入**阶段3**把全部角色/地点的定妆图一次性
生成完并逐条核对"一个不落"，确认无误后才进入阶段4（大场景切分）。
阶段5详细规划中途回补出的新角色/地点、以及按需生成的外观变体定妆图，
仍然保留"用到哪个大场景就顺带生成"的按需逻辑，放在阶段6里处理，
详见 `references/02_global_assets.md` 与 `references/05_assets_and_audio.md`。

**关键点：阶段5-7 是"以单个大场景为循环体"跑的，不是"全部大场景先
过完阶段5、再全部过阶段6、再全部过阶段7"的批处理流水线**——用户需要
每做完一个大场景就能看到、检查这个场景的成片，而不是等到最后一刻才看
到第一份可检查的结果。阶段3（全局定妆图集中生成）不受这条规则约束，
它和阶段2一样是发生在循环开始之前的全局一次性操作；阶段6里"仅针对
新增实体/变体"的按需补生成同理，跟随触发它的那个大场景走，详见 §2.3。
...
├── script.md                   # 阶段0产出的剧本（旁白/对话/说话人结构化），阶段2/4/5的输入源
├── script_review.yaml          # 阶段1 Agent 剧本核查结论（剧情覆盖/对话覆盖/说话人归属/无臆造）
├── global/
...
│   ├── consistency_report.yaml # 阶段7 Step 0 Agent 语义核查结论（角色/地点/情节一致性），Step 0.5 校验其完整/全pass/未过期后才能生成视频
...
**`novel_project.json`**（阶段2创建，全程只读，除非用户明确要求改
项目级配置）：
...
| `art_style` | string | 全书统一美术风格英文描述，阶段2 Step1 填 |
...
的一句话英文短语，阶段3生成定妆图时优先用它作为 prompt，阶段7每条
`prompt_en` 也引用它保证角色外观跨场景不漂移)、`voice_profile`
(string)、`first_appear` (string)、`relations` (string[]，如
`"char_02:挚友"`)、`asset_path` (string \| null，阶段3首次批量生成
时回填，阶段6按需补生成新实体时同样回填)、`face_reference_id`
(预留字段，恒 null，未实现)、`appearance_variants` (object[]，可选，
默认空数组，只有原文明确交代持续性外观变化——换装/变装/季节变化/
受伤等——才登记，见 `01_entity_extraction.md` Step 2.6；单条结构：
`variant_id` (`var_NN`)、`label_zh`、`trigger_zh` (原文依据，必填)、
`visual_override_en` (在不可变核心特征基础上覆盖服装/发型部分的完整
英文描述)、`applies_scope` (string[]，生效的 `macro_id`/`micro_id`
列表)、`asset_path` (string \| null，可选，阶段6按需生成))。
...
视觉细节)、`asset_path` (string \| null，阶段3首次批量生成时回填，
阶段6按需补生成新实体时同样回填)、`appearance_variants` (object[]，
可选，结构同角色，用于登记同一地点持续性的环境变化——昼夜/天气/陈设等，
`asset_path` 阶段6按需生成）。
...
**`script_review.yaml`**（阶段1由 Agent 写入，`check_script.py`
...
(`"pending"` → `"planned"` → `"done"`，阶段5/7分别推进)。时长约束是
...
| 字段 | 类型 | 由哪个阶段写入 | 取值/说明 |
|---|---|---|---|
| `id` | string | 阶段5 | `micro_NN`，全项目范围唯一 |
| `macro_id` | string | 阶段5 | 所属大场景 id |
| `uses_characters` / `uses_locations` | string[] | 阶段5 | 引用的全局库 id |
| `visual_hint` | string | 阶段5 | 画面提示（中文，供阶段7写 prompt_en 参考 + 阶段7 Agent 核查 prompt_en 是否符合情节的依据，需覆盖关键画面元素，不能空泛） |
| `character_variant_overrides` / `location_variant_overrides` | object \| 不写 | 阶段5（可选） | `{实体id: variant_id}`，只有该场景应使用某个已登记的 `appearance_variants` 变体时才写，不写则用默认 `visual_anchor_en`，见 04 文档 Step 4.5 |
| `content_blocks` | object[] | 阶段5 | 见下方 `content_block` 结构 |
| `duration_sec` | number \| null | 阶段6回填 | 4-12 秒范围内，阶段5阶段写入时恒为 `null` |
| `prompt_en` | string | 阶段7（Agent 手写） | 阶段5不产出，脚本不代为生成，为空视频生成会直接失败；必须嵌入引用到的角色/地点 `visual_anchor_en`（或生效变体的 `visual_override_en`），见 06 文档"一致性铁律" |
| `video_mode` | string | 阶段7（Agent 手写） | `"reference"` \| `"keyframe"` \| `"text"`，不设置时脚本按 `"text"` 处理 |
| `status` | string | 阶段7回写 | `"pending"` → `"done"` \| `"failed"` |

**`macro_scene_XX/consistency_report.yaml`**（阶段7 Step 0 由 Agent
...
**音频文件命名**（阶段6写入 `macro_scene_XX/audio/`，统一 `.wav`
...
**视频 clip 命名**（阶段7写入 `macro_scene_XX/clips/`）：
...
`check_*.py` 脚本（阶段3例外，见 `references/02_global_assets.md`
里 Agent 自查的方式）。**校验不通过，绝对不允许进入下一阶段**，
也不允许把状态字段推进（比如 `pending`→`planned`）。不通过时回到本阶段
内部修正，重新写文件、重新校验，直到通过。这是保证整条流水线不带着
"看似完成实则有缺陷"的产物往下传的唯一手段。

**特别地，阶段7"画面语义是否和角色/地点档案、和情节原文一致"这类判断
...
阶段5（详细规划）在处理某个大场景时，如果发现引用了阶段2没抽取到的
角色/地点，**不是报错终止**，而是：
...
2. 加载子资源 `assets-and-audio`（`references/05_assets_and_audio.md`）
   的 Step 1，只给这几个新增实体生成定妆图（阶段3已经批量生成过的
   主表条目不受影响，不需要重新过一遍）；
3. 回到阶段5继续规划，重新校验。

这个回补循环必须在阶段5内部闭环完成，不能把"引用了不存在的资源"这种
问题遗留到阶段6/7才发现——那样定位问题的成本高很多。类似地，阶段6如果
发现某个 micro_scene 配音后时长超过 12 秒硬限，也不是丢给阶段7硬着头皮
生成，而是回阶段5把这个小场景拆成两个，再重新走阶段6。
...
`macro_scenes.yaml` 有几个大场景，阶段5-7 就要跑几轮，且**以"单个大
场景"为最小闭环单位**：对某个 `macro_scene_XX`，依次做完

```
阶段5详细规划(该场景) → 阶段6配音(该场景, --macro-id macro_XX)
→ 阶段7小场景视频+大场景内合成(该场景, --macro-id macro_XX)
→ 该场景 status=done → 向用户展示这个大场景的结果，等待反馈/确认
```

才能开始下一个大场景的阶段5。**禁止的反模式**：把全部大场景先一起跑完
阶段5（或阶段6的批量配音，不带 `--macro-id`），攒够所有大场景的配音后
再统一进入阶段7——这样用户要等到最后才能看到第一份可检查的成片，一旦
早期大场景的规划/人设/配音方向有问题，返工成本是按全部大场景计算的，
而不是一个大场景。

例外（这些确实是全局一次性资源，跟"逐场景处理"不矛盾）：
- 阶段0的小说→剧本转换 + 阶段1剧本校验——全文一次性转换、一次性
  核查，不能只处理部分内容（场次之间的剧情连贯性需要通盘把握）；
- 阶段2的角色/地点抽取——基于 `script.md` 全文通读一次抽出全局角色
  地点表，不能只看单个大场景（会漏掉后面场景才出现的角色）；
- **阶段3的全局角色/地点定妆图集中生成**——对阶段2产出的主表逐条
  生成，一次性做完并校验"全部完成"，不按大场景拆分处理，也不允许
  留着一部分等某个大场景用到才生成（这正是阶段3独立出来的原因，见
  `references/02_global_assets.md`）；
- 阶段4的大场景切分——同样需要看剧本全文划分边界；
- 阶段6 Step 1 里**仅针对阶段5回补出的新角色/地点、以及被引用到的
  外观变体**的按需补生成——这部分不属于阶段3的批量生成范围，天然
  应该在处理第一个用到该新实体/变体的大场景时顺带生成，之后其它大
  场景直接复用，不用重复生成，也不需要、不应该攒到所有大场景一起
  生成。

阶段6 Step 2（配音）、阶段7（视频生成+合成）**默认都要带
`--macro-id macro_XX` 只处理当前这一个大场景**，不要在还没看到当前
大场景成片之前就去动下一个大场景。每跑完一个大场景的合成并校验通过、
`status=done` 后，把这个大场景的产物（时长、配音引擎使用情况、任何
持续性失败场景）汇报给用户，用户确认没问题或提完修改意见处理完后，
再开始下一个大场景的阶段5。
...
（视频生成）、或直接调用 `gen_image_with_text`（定妆图，阶段3的批量
生成 / 阶段6的按需补生成）这几步时，不可避免会遇到 API 报错（限流/
参数错误/网络问题）。处理规范见子资源 `error-handling`
（`references/error_handling.md`），核心原则一句话说完：**限流类错误脚本会
...
`script.md` 校验通过后才进入阶段2（角色/地点抽取），抽取完的主表通过
阶段3（`references/02_global_assets.md`）集中生成定妆图并校验完成后，
才进入阶段4，再按上面的阶段顺序推进。同时在
...
- 已知坑：edge-tts 对超过 40 字的单段文本经常 300s 超时，配音时长的
  content_blocks 尽量控制在 40 字以内，超过的提前在阶段5多拆一刀。
- 下一步：从 macro_02 开始阶段5详细规划。
```
