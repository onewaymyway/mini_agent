...
- **`script_source` 硬性要求**：脚本禁止把本次变换观察到的具体文本结果
  写死进返回值，`run()` 必须真正对传入的 `input.content.text` 做变换，
  换一段不同的输入文本要能得到不同的输出。提交前自检：如果 `content.text`
  换成别的文本，这段代码返回的还是同一个结果吗？如果是，说明数据被写死了，
  不能提交。
