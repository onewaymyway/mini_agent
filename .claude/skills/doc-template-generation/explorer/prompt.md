...
- **`script_source` 硬性要求**：脚本禁止把本次探索样例的具体生成结果写死
  进返回值，`run()` 必须真正基于传入的 `input.content` 重新抽取/分段/渲染，
  换一份不同的样例内容要能产生不同的文档结构。提交前自检：如果 `content`
  换成别的样例，这段代码返回的还是同一份文档吗？如果是，说明数据被写死了，
  不能提交。
